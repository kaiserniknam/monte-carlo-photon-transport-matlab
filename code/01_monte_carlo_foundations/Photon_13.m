...
% the same as
% "Photon_Packet_Transport_in_a_Scattering_and_Absorbing_Medium_11" but to replicate
% On the geometry dependence of differential pathlength factor for near-infrared spectroscopy. I. Steadystate with homogeneous medium
% a sample
...
for i_a = 1:length(mua)
    for i_s = 1:length(mus)
        ...
        figure(8), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['\mu_s = ',num2str(mus(i_s)),', \mu_a = ',num2str(mua(i_a)), ' (MC)'],'Color',get_color(do_count)), hold on
        mus_p = mus(i_s)*(1-g);
        figure(8), plot(d_c,(sqrt(3*mus_p)/2/sqrt(mua(i_a))).*(d_c.*sqrt(3*mus_p*mua(i_a)))./(d_c.*sqrt(3*mus_p*mua(i_a))+1),'LineWidth',2,'DisplayName',['\mu_s = ',num2str(mus(i_s)),', \mu_a = ',num2str(mua(i_a)), ' (Analytic)'],'LineStyle','-.','Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s/d (cm)'), title('~ DPF')
        set(gca,'fontsize',24), axis tight, grid on, legend show, hold on
