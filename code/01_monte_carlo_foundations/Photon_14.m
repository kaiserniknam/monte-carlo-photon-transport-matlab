...
% the same as "Photon_Packet_Transport_in_a_Scattering_and_Absorbing_Medium_13" but to test Dr. Das's idea for breast tumor
...
for i_var = [0,1]
    var = {i_var,0,0,1,0.50,0.50,0.50};
    [d, s, w, no_of_surf_photons, no_of_dpth_photons] = do_simulation (var);
    clearvars var
    ...
    figure(1), plot(d,s,'o','MarkerFaceColor',get_color(i_var+1),'MarkerEdgeColor','k','DisplayName',get_legend(i_var))
    hold on, plot([0 max(d)],[0 max(d)],'k-.')
    ...
    figure(2), plot(s_f,-log10(w_f),'o','MarkerFaceColor',get_color(i_var+1),'MarkerEdgeColor','k','DisplayName',get_legend(i_var))
    ...
    figure(3), plot(d_c,-log10(w_mean),'LineWidth',2,'DisplayName',get_legend(i_var))
    xlabel('d (cm)'), ylabel('OD (unitless)'), title('OD vs. d')
    set(gca,'fontsize',16), axis tight, grid on, legend show, hold on

    figure(4), loglog(d_c,-log10(w_mean)./d_c,'LineWidth',2,'DisplayName',get_legend(i_var))
    xlabel('d (cm)'), ylabel('OD/d (cm^{-1})'), title('OD/d vs. d')
    set(gca,'fontsize',16), axis tight, grid on, legend show, hold on

    figure(5), errorbar(d_c,s_mean,s_var,'LineWidth',2,'DisplayName',get_legend(i_var))
    xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
    set(gca,'fontsize',16), axis tight, grid on, legend show, hold on

    figure(6), plot(d_c,s_mean,'LineWidth',2,'DisplayName',get_legend(i_var))
    xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
    set(gca,'fontsize',16), axis tight, grid on, legend show, hold on

    figure(7), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',get_legend(i_var))
    xlabel('d (cm)'), ylabel('s/d (cm)'), title('~ DPF')
    set(gca,'fontsize',16), axis tight, grid on, legend show, hold on

    figure(8), plot(d_c,w_mean,'LineWidth',2,'DisplayName',get_legend(i_var))
    xlabel('d (cm)'), ylabel('Intensity (W.m^{-2})'), title('I vs. d')
    set(gca,'fontsize',16), axis tight, grid on, legend show, hold on
end
end

function [d, s, w, no_of_surf_photons, no_of_dpth_photons] = do_simulation (var)
...
%     % geometries
%     figure(11), pcolor(t_model.G.y,t_model.G.z,squeeze(t_model.MC.M(61,:,:)).'), axis ij, colormap parula, caxis([1 3])
%     xlabel('y (cm)'), ylabel('z (cm)'), set(gca,'fontsize',24), axis equal, axis tight
...
function [out] = get_legend(idx)
if idx==0
    out = 'Breast w/o Tumor';
else
    out = 'Breast w Tumor';
end
end
