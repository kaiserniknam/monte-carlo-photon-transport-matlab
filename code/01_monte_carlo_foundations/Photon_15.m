...
% the same as "Photon_Packet_Transport_in_a_Scattering_and_Absorbing_Medium_13" but to replicate
% but for a better data extraction and intensity recording (angle of photons)
...
mus = [100];
set_of_lim = nan(size(mua));

for i_a = 1:length(mua)
    for i_s = 1:length(mus)
        do_count = do_count + 1;
        [d, s, w, a, no_of_surf_photons, no_of_dpth_photons] = do_simulation (mua(i_a),mus(i_s),g,n,var);
        ...
        i_mean = accumarray(ind_d,w.*(-a),[],@sum); i_mean = i_mean/(no_of_surf_photons+no_of_dpth_photons);
        ...
        figure(2), plot(s_f,-log10(w_f),'o','MarkerFaceColor',get_color(do_count),'MarkerEdgeColor','k','DisplayName',['\mu_s = ',num2str(mus(i_s)),', \mu_a = ',num2str(mua(i_a))])
        ...
        figure(8), plot(d_c,-log10(i_mean)./d_c,'LineWidth',2,'DisplayName',['\mu_s = ',num2str(mus(i_s)),', \mu_a = ',num2str(mua(i_a))])
        xlabel('d (cm)'), ylabel('OD*cos(/theta)/d (cm^{-1})'), title('OD/d vs. d')
        set(gca,'fontsize',24), axis tight, grid on, legend show, hold on

        figure(9),
        [N,edges] = histcounts(s,0:2.5:100);
        bar(1/2*(edges(1:end-1)+edges(2:end-0)),log10(N),'FaceColor',get_color(do_count),'DisplayName',['\mu_s = ',num2str(mus(i_s)),', \mu_a = ',num2str(mua(i_a))])
        xlabel('s (cm)'), ylabel('log_{10}(freq)'), title('hist. of s')
        set(gca,'fontsize',24), axis tight, grid on, legend show, hold on
        [aa,bb] = findpeaks(diff(-log10(N))); [val,idx] = find(aa==max(aa(~isinf(aa)))); cc = 1/2*(edges(1:end-1)+edges(2:end-0));
        disp(['max s = ',num2str(cc(bb(idx)))])
        figure(1), plot(d,ones(size(d)).*cc(bb(idx)),'-','Color',get_color(do_count),'HandleVisibility','off')
        set_of_lim(i_a) = cc(bb(idx));
        clearvars aa bb cc val idx edges N

        clearvars d s a w d_c s_c s_mean w_mean i_mean s_var f_d f_s no_of_dpth_photons no_of_surf_photons
    end
end
figure(10)
plot(mua,set_of_lim,'LineWidth',2), hold on
plot(min(mua):0.01:max(mua),4.5./(min(mua):0.01:max(mua)),'-.','LineWidth',2), hold on
xlabel('\mu_a (cm^{-1})'), ylabel('s_{max} (cm)'), title('s_{max} vs. \mu_a'), hold off
set(gca,'fontsize',24), axis tight, grid on

end

function [d, s, w, a , no_of_surf_photons, no_of_dpth_photons] = do_simulation (mua,mus,g,n,var)
%% Geometry definition
...
% model.MC.farFieldRes = 1e3;                % The code stores calculate the far field direction angles of the escaping photons and store them in a 2D NxN array.
...
%% light collector
% model.MC.useLightCollector = true; % the MC simulation should keep track of whether photons hit the light collector/detector.
...
if     double(parameters{1}) == 0
    M(Z>=zSurface) = 2; % standard (breast) tissue
