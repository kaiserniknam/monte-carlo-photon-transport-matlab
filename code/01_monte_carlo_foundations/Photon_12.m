...
for i_s = 1:length(mu_s)
    for i_a = 1:length(mu_a)
        ...
        N_bin = 250; s_edges = linspace(min(s),max(s),N_bin+1); % # d bins
        s_c = 1/2*(s_edges(1:end-1)+s_edges(2:end-0))';
        [f_s,~,ind] = histcounts(s,s_edges); f_s = f_s./max(f_s);
        w_s = accumarray(ind,w,[],@sum); w_s = w_s./max(w_s);
        plot(s_c,w_s,'LineWidth',2,'DisplayName',['\mu_a = ',num2str(mu_a(i_a))]), hold on
        clearvars d s w no_of_surf_photons no_of_dpth_photons w_sum
    end
end
hold off, grid on, xlim([0 0.5]), title('normalized intensity vs. s'), xlabel('s (cm)'), ylabel('normalized intensity')
legend('show'), xlim([0 0.15])
set(gca,'FontSize',24)
end
...
while size(scatter_stat,1) < 100000
