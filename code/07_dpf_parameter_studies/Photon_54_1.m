...
for i_a = 1:length(set_of_mua)
    for i_s = 1:length(set_of_mus)
        for i_z = 1:length(set_of_Lzs)
            ...
            % read dbase
            the_filename = ['/home/kaiser/BackUp/Dropbox/research/2.NIRs Project/code/DB/Photon_54_mua_',sprintf('%.2f',set_of_mua(i_a)),'_mus_',sprintf('%.2f',set_of_mus(i_s)),'_Lz_',num2str(set_of_Lzs(i_z)),'.mat' ];
...
set(gca,'fontsize',24), axis square, axis tight
legend('show','Location','northeast')
save('mus_50_250.mat','set_of_mua','set_of_mus','g','n','set_of_Ts','set_of_Lzs')
end
