...
% this version: to plot sensivity map
% The sensitivity heatmap was computed as the point-wise product of
% the fluence rate and the probability distribution of photon launch
% positions that ultimately reached the detector fiber.
...
Lx = 19.1; Ly = 19.1; Lz = 06.0; % The size of the computational domain (in cm) to prevent reflections from the boundaries.
...
n = 1.4; g = 0.9;
set_of_mua = [0.3341    ]; % 1/cm
set_of_mus = [300, 159.8]; % 1/cm
...
for i_a = 1:length(set_of_mua)
    for i_s = 1:length(set_of_mus)
        ...
        the_filename = ['Photon_61_mua_',sprintf('%.2f',set_of_mua(i_a)),'_mus_',sprintf('%.2f',set_of_mus(i_s)),'_fluence.mat'];
        if ~exist(the_filename,'file')
            ...
            save  (the_filename, ...
                'x_in', 'y_in', 'z_in', 'x_ot', 'y_ot', 'z_ot','s', 'w', 'no_of_photons', 'M_raw','mua','mus','g','n')
            clearvars x_in y_in z_in x_ot y_ot z_ot s w no_of_photons M_raw
        end
        clearvars the_filename

        the_filename = ['Photon_61_mua_',sprintf('%.2f',set_of_mua(i_a)),'_mus_',sprintf('%.2f',set_of_mus(i_s)),'_probability.mat'];
        if ~exist(the_filename,'file')
            [x_in, y_in, z_in, x_ot, y_ot, z_ot, s, w, no_of_photons, M_raw] = do_simulation_2 ( ...
                [mua,mus,g   ,n  ], ...
                [nan,nan,nan ,nan], ...
                [Lx,Ly,Lz],[nan,nan,nan],[nan,nan,nan],z_air,001,nExamplePaths);
            save  (the_filename, ...
                'x_in', 'y_in', 'z_in', 'x_ot', 'y_ot', 'z_ot','s', 'w', 'no_of_photons', 'M_raw','mua','mus','g','n')
            clearvars x_in y_in z_in x_ot y_ot z_ot s w no_of_photons M_raw
        end
...
function [x_in, y_in, z_in, x_ot, y_ot, z_ot, s, w, no_of_photons, M_raw] = do_simulation_2 (opt_bckg,opt_sgnl,cmd_size,sig_pos,sig_size,zSurface,wvlngth,nExamplePaths)
%% Geometry definition
model = MCmatlab.model;
dl = 0.1; % spatial resolution (cm)
model.G.nx                = round(cmd_size(1)./dl);  % Number of bins in the x direction
...
model.MC.matchedInterfaces        = false;   % Assumes all refractive indices are the same
model.MC.boundaryType             = 1;       % 0: No escaping boundaries, 1: All cuboid boundaries are escaping, 2: Top cuboid boundary only is escaping, 3: Top and bottom boundaries are escaping, while the side boundaries are cyclic
model.MC.nPhotonsRequested = 500;            % The time to run the MC simualtion for, in minutes. The number of photos launched will vary from run to run.
model.MC.nExamplePaths = 500;                % The code stores the paths of the first N photons for subsequent visualization during the plotting.
model.MC.wavelength = wvlngth;               % [nm] Excitation wavelength, used for determination of optical properties for excitation light
clearvars wvlngth nPhotonsReq

%% For a pencil beam, the "focus" is just a point that the beam goes through, here set to be the center of the cuboid:
model.MC.lightSource.sourceType = 1;   % radial-factorizable beam
...
while size(scatter_stat,1) < nExamplePaths
    t_model = model;
    % select a random point inside breast
    t_model.MC.lightSource.xFocus = -t_model.G.Lx/2 + t_model.G.Lx*rand;
    t_model.MC.lightSource.yFocus = -t_model.G.Ly/2 + t_model.G.Ly*rand;
    t_model.MC.lightSource.zFocus = 0               + t_model.G.Lz*rand;
    t_model.MC.lightSource.theta        = acos(2*rand-1);   % [rad] Polar angle of beam center axis
    t_model.MC.lightSource.phi          = 2*pi*rand;   % [rad] Azimuthal angle of beam center axis
    M_raw = t_model.G.M_raw;
    t_model = runMonteCarlo(t_model);
    % t_model = plot(t_model,'MC');
    % do calc
    spratrs = find(isnan(t_model.MC.examplePaths(1,:)));
    strt_pnts = spratrs(2:2:end); strt_pnts = strt_pnts(1:end-1);
    ...
    no_of_photons = no_of_photons + size(t_scatter_stat,1);
    scatter_stat = [scatter_stat ; t_scatter_stat];
    if mod(size(scatter_stat,1),100)
        disp('****************************************************')
        disp(['probability progress ... ',num2str(size(scatter_stat,1)/nExamplePaths*100),'%'])
        disp('****************************************************')
    end
    % clearvars surf_photons t_scatter_stat
end
