...
no_of_layer = 6;             % number of gradient layers
...
n_lyr = linspace(1.44, 1.35, no_of_layer);
mus_lyr = 30.*((1.46./n_lyr - 1)./(1.46./1.35 - 1)+eps).^2; % 1/cm
...
for i_stat = 1:length(set_of_status)
    ...
    the_filename = ['Photon_63_status_',sprintf('%.2f',set_of_status(i_stat)),'.mat'];
    if ~exist(the_filename,'file')
        ...
        [x_in, y_in, z_in, x_ot, y_ot, z_ot, s, w, no_of_photons, M_raw] = do_simulation ( ...
            set_of_mua,set_of_mus,set_of___g,set_of___n,z_edges, ...
            [Lx,Ly,Lz],001,nPhotonsReq,nExamplePaths,beam_X,beam_Y,beam_phi,beam_tht);
        save  (the_filename, ...
            'x_in', 'y_in', 'z_in', 'x_ot', 'y_ot', 'z_ot','s', 'w', 'no_of_photons', 'M_raw', 'set_of_mua', 'set_of_mus', 'set_of___n', 'set_of___g', 'z_edges')
        clearvars x_in y_in z_in x_ot y_ot z_ot s w no_of_photons M_raw
        clearvars set_of___g set_of___n set_of_mus set_of_mua
    end
...
function [x_in, y_in, z_in, x_ot, y_ot, z_ot, s, w, no_of_photons, M_raw] = do_simulation (mua,mus_lyr,g,n_lyr,z_edges,cmd_size,wvlngth,nPhotonsReq,nExamplePaths,beam_X,beam_Y,beam_phi,beam_tht)
%% Geometry definition
...
model.G.mediaPropParams     = {mua,mus_lyr,g,n_lyr,z_edges}; % A cell array that you can use to contain all sorts of inputs you would like to use inside the mediaPropertiesFunc
...
% extract parameters
set_of_mua = double(var{1});
set_of_mus = double(var{2});
set_of___g = double(var{3});
set_of___n = double(var{4});
