...
% the source is inside the tissue (oxygen sensing version)
...
beam_tht = -pi;   % azimuthal angle of beam in cm
beam_X = 0.0;     % X deviation of beam in cm
beam_Y = 0.0;     % Y deviation of beam in cm

n = 1.4; g = 0.95;
set_of_mua = (0.2:0.1:0.5); % mu_a
set_of_mus = (100:100:400); % mu_s
set_of_bmZ = (1.0:1.0:3.0); % Z deviation of beam in cm

for i_z = 1:length(set_of_bmZ)
    for i_a = 1:length(set_of_mua)
        for i_s = 1:length(set_of_mus)
            mua = set_of_mua(i_a);
            mus = set_of_mus(i_s);
            beam_Z = set_of_bmZ(i_z);
            the_filename = ['Photon_34_beamZ_',sprintf('%.1f',set_of_bmZ(i_z)),'_mua_',sprintf('%.1f',set_of_mua(i_a)),'_mus_',sprintf('%.0f',set_of_mus(i_s)),'.mat' ];
            if ~exist(the_filename,'file')
                [x, y, z, d, s, w, c, a, no_of_photons, M_raw] = do_simulation ( ...
                    [mua,mus,g   ,n  ], ...
                    [nan,nan,nan ,nan], ...
                    [Lx,Ly,Lz],[nan,nan,nan],[nan,nan,nan],z_air,001,nPhotonsReq,nExamplePaths,beam_X,beam_Y,beam_Z,beam_phi,beam_tht);

                save  (the_filename, ...
                    'x', 'y', 'z', 'd', 's', 'w', 'c', 'a', 'no_of_photons', 'M_raw','mua','mus','beam_X','beam_Y','beam_Z','beam_tht','beam_phi')

                clearvars x y z d s w c a no_of_photons M_raw
            end
            clearvars mua mus beam_Z
...
function [x, y, z, d, s, w, c, a, no_of_photons, M_raw] = do_simulation (opt_bckg,opt_sgnl,cmd_size,sig_pos,sig_size,zSurface,wvlngth,nPhotonsReq,nExamplePaths,beam_X,beam_Y,beam_Z,beam_phi,beam_tht)
%% Geometry definition
...
% model = plot(model,'G');
clearvars opt_bckg opt_sgnl cmd_size sig_pos sig_size dl zSurface
