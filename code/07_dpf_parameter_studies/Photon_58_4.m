...
set_of_mua = 0.0275;
set_of_mus = [(30:50)./4,(30:50),(30:50).*4];
...
for i_a = 1:1:length(set_of_mua)
    for i_s = 1:1:length(set_of_mus)
        ...
        TheColor = get_color(i_s/length(set_of_mus)); % make my own colormap

        % read dbase
        the_filename = ['/home/kaiser/BackUp/Dropbox/research/2.NIRs Project/code/DB/Photon_58_mua_',sprintf('%.4f',set_of_mua(i_a)),'_mus_',sprintf('%.2f',set_of_mus(i_s)),'.mat'];
...
figure
plot(set_of_mus,rsq_of_diff,'LineWidth',2)
axis([[min(set_of_mus) max(set_of_mus) 0.90 1.00]])
xlabel('\mu_s (cm^{-1})'), set(gca,'xtick',[min(set_of_mus) 3/4*min(set_of_mus)+1/4*max(set_of_mus) 2/4*min(set_of_mus)+2/4*max(set_of_mus) 1/4*min(set_of_mus)+3/4*max(set_of_mus) max(set_of_mus)])
ylabel('R^2 (unitless)'),  set(gca,'ytick',0.90:0.01:1.00)
title('the evolution of R^2 over \mu_s'),
set(gca,'fontsize',20), axis square, grid on
end
...
function [out] = get_color(mus_norm)
out = [mus_norm 0 1-mus_norm];
end
