...
% ref: Sergio Fantini, Scott A. Walker, Maria Angela Franceschini, Michael Kaschke, Peter M. Schlag, and K. Thomas Moesta, "Assessment of the size, position, and optical properties of breast tumors in vivo by noninvasive optical methods," Appl. Opt. 37, 1982-1989 (1998)
...
set_of_beam_X = -3:1:+3;               % X deviation of beam in cm
set_of_beam_Y = -3:1:+3;               % Y deviation of beam in cm
    ...
    for i_BeamY = 1:length(set_of_beam_Y)
        for i_czs = 1:length(set_of_czs)
            for i_wvl = 1:length(set_of_wvl)
                ...
                the_filename = ['Photon_56_X_',num2str(beam_X),'_Y_',num2str(beam_Y),'_cz_',num2str(cz),'_wvlngth_',num2str(wvlngth),'.mat' ];
...
model.G.geomFuncParams      = {zSurface,2,3,4,sgn_HbO2(1),sgn_HbO2(2),sgn_HbO2(3),sgn_dxHb(1),sgn_dxHb(2),sgn_dxHb(3),sig_size(1),sig_size(2),sig_size(3)}; % A cell array that you can use to contain all sorts of inputs you would like to use inside the geomFunc
% model = plot(model,'G');
