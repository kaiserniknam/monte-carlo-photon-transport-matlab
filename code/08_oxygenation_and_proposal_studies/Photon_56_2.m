...
% Population Analysis: Optical Density (OD) and Δ (Difference to no signal) Results
...
N_bin = 101;
set_of_base = nan(length(set_of_beam_X),length(set_of_beam_Y),length(set_of_wvl),length(set_of_czs),N_bin,N_bin);
set_of_data = nan(length(set_of_beam_X),length(set_of_beam_Y),length(set_of_wvl),length(set_of_czs),N_bin,N_bin);
    ...
    for i_BeamY = 1:length(set_of_beam_Y)
        for i_czs = 1:length(set_of_czs)
            for i_wvl = 1:length(set_of_wvl)
                ...
                [x_c,y_c,set_of_data(i_BeamX,i_BeamY,i_wvl,i_czs,:,:)] = make_2d(t_db,Lx,Ly,N_bin);
                eval(['t_db = load(''/home/kaiser/BackUp/Dropbox/research/2.NIRs Project/code/DB/Photon_',num2str(code_num),'_null_X_',num2str(beam_X),'_Y_',num2str(beam_Y),'_cz_',num2str(cz),'_wvlngth_',num2str(wvlngth),'.mat'');'])
                [x_c,y_c,set_of_base(i_BeamX,i_BeamY,i_wvl,i_czs,:,:)] = make_2d(t_db,Lx,Ly,N_bin);
                clearvars beam_X beam_Y cz wvlngth
            end
        end
    end
end
clearvars i_BeamX i_BeamY i_csz i_vwl code_num t_db


% plot OD
N_lines = 51; [set_of_lines,my_colormap] = make_colormap(-3,+3,N_lines);
for i_BeamX = 1:length(set_of_beam_X)
    for i_BeamY = 1:length(set_of_beam_Y)
        figure
        for i_czs = 1:length(set_of_czs)
            for i_wvl = 1:length(set_of_wvl)
                ...
                cz = set_of_czs(i_czs);
                wvlngth = set_of_wvl(i_wvl);
                subplot(2,2,(i_czs-1).*length(set_of_czs)+i_wvl)

                contourf(x_c,y_c,-log(squeeze(set_of_data(i_BeamX,i_BeamY,i_czs,i_wvl,:,:))).'+log(squeeze(set_of_base(i_BeamX,i_BeamY,i_czs,i_wvl,:,:))).',N_lines,'LineColor','none','LineWidth',0.1)
                hold on, rectangle('Position',[-rx-dx/2 -ry  2*rx+eps 2*ry+eps],'Curvature',[1 1],'LineWidth',2,'LineStyle','-','EdgeColor','r'), hold off % breast area
                hold on, rectangle('Position',[-rx+dx/2 -ry  2*rx+eps 2*ry+eps],'Curvature',[1 1],'LineWidth',2,'LineStyle',':','EdgeColor','k'), hold off % breast area
