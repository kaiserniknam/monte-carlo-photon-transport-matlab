...
for i_concentration = 1:length(set_of_concentration)
    for i_rep = set_of_verse
        ...
        if isnan(the_concentration)
            ...
            if the_concentration <= 0.05
                mua_incl = max(0.0275,-0.66+77.81.*the_concentration); g_incl = 0.90; n_incl = 1.34; mus_incl = 6./(1-g_incl);
            else
                mua_incl = max(0.0275, 3.44+ 3.66.*the_concentration); g_incl = 0.90; n_incl = 1.34; mus_incl = 6./(1-g_incl);
            end
        end
        the_filename = ['Photon_80_concentration_',num2str(the_concentration),'_',num2str(i_rep),'.mat'];
