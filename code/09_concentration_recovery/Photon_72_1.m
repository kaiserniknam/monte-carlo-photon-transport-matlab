...
for i_concentration = 1:length(set_of_concentration)
    ...
    the_filename = ['/home/kaiser/BackUp/Dropbox/research/2.NIRs Project/code/DB/Photon_72_concentration_',num2str(the_concentration),'.mat' ];
...
for i_c = 2:length(set_of_concentration)
    the_color = [(i_c-1)./(length(set_of_concentration)-1) 0 1-(i_c-1)./(length(set_of_concentration)-1)];
    figure(7)
    mua = set_of_mua(i_c);
    X = squeeze(set_of_ODs(i_c,:,1));
    Y = squeeze(set_of_ODs(i_c,:,2));
    plot(X,-log(Y)+log(Y(1)),'LineStyle','-','LineWidth',2,'Color',the_color,'DisplayName',num2str(set_of_concentration(i_c))), hold on
    Yhat = ...
        exp(params_A(1)).*(mua).^(1+params_A(2)).*X + ...
        exp(params_B(1)).*(mua).^(1+params_B(2)) + ...
        exp(params_C(1)).*(mua).^(1+params_C(2)).*log(X);
    plot(X,Yhat,'LineStyle',':','LineWidth',2,'Color',the_color,'HandleVisibility','off'), hold on
    clearvars the_color mua X Y Yhat
end
xlabel('d (cm)'), ylabel('OD (a.u.)'), set(gca,'fontsize',16)
title('Effect of Inclusion Concentration on OD vs. Models'), grid off, axis tight square
clearvars i_c
end
