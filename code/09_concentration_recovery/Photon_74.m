...
%
% Based on Version 71, better unrestricted absorption coefficient (mu_a)
...
for i_concentration = 1:length(set_of_concentration)
    ...
    if isnan(the_concentration)
        ...
        if the_concentration <= 0.07
            mua_incl = max(0.0275,-0.62+75.68.*the_concentration); g_incl = 0.90; n_incl = 1.34; mus_incl = 6./(1-g_incl);
        else
            mua_incl = max(0.0275, 3.43+17.45.*the_concentration); g_incl = 0.90; n_incl = 1.34; mus_incl = 6./(1-g_incl);
        end
    end
    the_filename = ['Photon_74_concentration_',num2str(the_concentration),'.mat'];
