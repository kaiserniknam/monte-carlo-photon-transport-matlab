...
%
%   Based on Version 65, with improved absorption coefficient (mu_a) estimation derived from experimental measurements
...
for i_concentration = 1:length(set_of_concentration)
    ...
    if isnan(the_concentration)
        ...
        if the_concentration <= 0.06
            mua_incl = max(0.0275,-0.55+67.15.*the_concentration); g_incl = 0.90; n_incl = 1.34; mus_incl = 6./(1-g_incl);
        else
            mua_incl = 3.63;                                       g_incl = 0.90; n_incl = 1.34; mus_incl = 6./(1-g_incl);
        end
    end
    the_filename = ['Photon_68_concentration_',num2str(the_concentration),'.mat' ];
