...
% This Version: Intralipid 0.5% experimental phantom (India ink tumor inclusion in gelatin 10%)
% Configuration:
%   Background: Intralipid 0.5% scattering matrix
%   Inclusion: Localized absorbing tumor (India ink embedded in gelatin 10%)
% Purpose:
%   Validation of DPF-based absorption estimation under experimentally realistic conditions.
% Based on Version 82, better optical properties (mu_a, mu_s, n, and g)
...
% Geometry
Lx = 19.1; Ly = 19.1; Lz = 5.30; % The size of the computational domain (in cm) to prevent reflections from the boundaries.
cx = 0.00; cy = 0.00; cz = 1.20; % Tumor depth from the skin (in cm)
...
for i_concentration = 1:length(set_of_concentration)
    ...
    mua_phnt = 0.000068;                           g_phnt = 0.80;   n_phnt = 1.33;   mus_phnt = 11.4;
    if isnan(the_concentration)
        mua_incl = mua_phnt;                       g_incl = g_phnt; n_incl = n_phnt; mus_incl = mus_phnt;
    else
        mua_incl = 0.0281+32.4.*the_concentration; g_incl = 0.00;   n_incl = 1.33;   mus_incl = 0.002;
    end
    the_filename = ['Photon_83_concentration_',num2str(the_concentration),'.mat'];
