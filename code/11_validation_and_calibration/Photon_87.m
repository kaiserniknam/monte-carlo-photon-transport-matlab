...
cx = NaN;  cy =  NaN; cz = NaN;  % Tumor depth from the skin (in cm)
rx = NaN;  ry =  NaN; rz = NaN;  % Tumor radius within the breast (in cm)(1.0 to 2.5 cm).
...
for i_musphant = 1:length(set_of_musphant)
    for i_rep = set_of_repetition
        mua_phnt = 0.0275;   g_phnt = 0.93;   n_phnt = 1.33;   mus_phnt = 55.0.*set_of_musphant(i_musphant);
        mua_incl = mua_phnt; g_incl = g_phnt; n_incl = n_phnt; mus_incl = mus_phnt;
        the_filename = ['Photon_87_concentration_',num2str(set_of_musphant(i_musphant),'%.2f'),'_',num2str(i_rep),'.mat'];
