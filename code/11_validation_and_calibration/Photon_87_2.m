...
% Based on Version 79
% data analysis: w & w/o initial filtration
...
for i_musphant = 1:length(set_of_musphant)
    ...
    plot(x_bind,-log(y_bind),'LineWidth',2,'LineStyle','-','Color',the_color,'DisplayName',['C = ',num2str(set_of_musphant(i_musphant),'%.2f'),': method 1']), hold on
    clearvars x_bind y_bind
    [y_bind,x_bind] = get_OD_prime (the_filename,dlta_d);
    plot(x_bind,-log(y_bind),'LineWidth',4,'LineStyle',':','Color',the_color,'DisplayName',['C = ',num2str(set_of_musphant(i_musphant),'%.2f'),': method 2']), hold on
    clearvars x_bind y_bind
end
xlabel('separation (cm)')
ylabel('OD (a.u.)')
grid on, axis square, hold off, axis([0 8 2 12])
set(gca,'fontsize',20), legend('show','Location','southeast','NumColumns',2,'orientation','horizontal')
...
function [y_bind,x_bind] = get_OD_prime (the_filename,dlta_d)
Lx = 19.1; Ly = 19.1; Lz = 5.3;  % The size of the computational domain (in cm) to prevent reflections from the boundaries.
t_db = load(the_filename); clearvars the_filename
...
[~,~,index_in] = histcounts(sqrt(t_db.x_ot(t_db.z_ot<0).^2+t_db.y_ot(t_db.z_ot<0).^2+t_db.z_ot(t_db.z_ot<0).^2),d_diff_edges); % d_diffuse bins
clearvars d_diff_edges Lx Ly Lz

% I vs. d
fun_x = @mean; fun_y = @sum; TheCode = 0;
x_temp = sqrt(t_db.x_ot(t_db.z_ot<0).^2+t_db.y_ot(t_db.z_ot<0).^2+t_db.z_ot(t_db.z_ot<0).^2);
y_temp = t_db.w(t_db.z_ot<0);
x_bind = accumarray(index_in,x_temp,[],fun_x,nan);
y_bind = accumarray(index_in,y_temp,[],fun_y,nan);
y_bind = ...
    (            y_bind./(pi.*dlta_d.*dlta_d + 2.*pi.*x_bind.*dlta_d)) ./ ...
