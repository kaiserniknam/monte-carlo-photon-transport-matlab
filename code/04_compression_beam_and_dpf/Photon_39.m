...
set_of_beam_X = [0.5,1.0,1.5]; % X deviation of beam in cm
set_of_beam_Y = [0.5,1.0,1.5]; % Y deviation of beam in cm
...
for i_BeamX = 1:length(set_of_beam_X)
    for i_BeamY = 1:length(set_of_beam_Y)
        for i_scale = 1:length(set_of_scal)
            ...
            save  (['Photon_39_X_',num2str(beam_X),'_Y_',num2str(beam_Y),'_phi_',num2str(beam_phi),'_tht_',num2str(beam_tht),'_sclae_',sprintf('%.1f',set_of_scal(i_scale)),'.mat' ], ...
                'x', 'y', 'd', 's', 'w', 'a', 'no_of_surf_photons', 'no_of_dpth_photons', 'M_raw', ...
                'w_mean','d_mean','s_mean','s_var','N','N_samples','x_c','y_c','beam_X','beam_Y','beam_tht','beam_phi')
            clearvars ind_x ind_y i_sample N_samples beam_X beam_Y
