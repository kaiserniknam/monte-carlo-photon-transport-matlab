...
% this version: s vs. d for breast tissue (compression-induced hemodynamic changes in breast tissue)
...
for idx = 1
    ...
    for i_scale = 1:length(set_of_scal)
        ...
        [d, s, w, a, no_of_surf_photons, no_of_dpth_photons, M_raw] = do_simulation ( ...
            [mua_brst,mus_brst,g_brst,n_brst], ...
            [mua_tumr,mus_tumr,g_tumr,n_tumr], ...
            [Lx,Ly,Lz],[cx,cy,cz/set_of_scal(i_scale)], ...
            [rx*sqrt(set_of_scal(i_scale)),ry*sqrt(set_of_scal(i_scale)),rz/set_of_scal(i_scale)],z_air,wvlnt,nPhotonsReq,nExamplePaths);
        ...
        plot(d,s,'o','MarkerFaceColor',get_color(do_count),'MarkerEdgeColor','k','DisplayName',['scale = ',num2str(set_of_scal(i_scale))])
        ...
        saveas(gcf,['Photon_27_',sprintf('%.1f',set_of_scal(i_scale)),'.tiff'],'tif')
        save  (    ['Photon_27_',sprintf('%.1f',set_of_scal(i_scale)),'.mat' ],'d', 's', 'w', 'a', 'no_of_surf_photons', 'no_of_dpth_photons', 'M_raw')

        figure(3), plot(d_c,s_mean,'LineWidth',2,'DisplayName',['scale = ',num2str(set_of_scal(i_scale))],'Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
        set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

        figure(4), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['scale = ',num2str(set_of_scal(i_scale))],'Color',get_color(do_count)), hold on
        ...
        semilogx(1/2*(edges_s(1:end-1)+edges_s(2:end-0)),N_s,'Color',get_color(do_count),'DisplayName',['scale = ',num2str(set_of_scal(i_scale))]), hold on
...
function [d, s, w, a, no_of_surf_photons, no_of_dpth_photons,M_raw] = do_simulation (opt_bckg,opt_sgnl,cmd_size,sig_pos,sig_size,zSurface,wvlngth,nPhotonsReq,nExamplePaths)
%% Geometry definition
