...
% plot s
figure(1), N_lines = 15; [set_of_lines,my_colormap] = make_colormap(0,50,N_lines);
for i_scale = 1:length(set_of_scal)
    subplot(4,6,i_scale)
    eval(['t_db = (db_',strrep(sprintf('%.1f',set_of_scal(i_scale)),'.','_'),'.s_mean);']);
    [xq,yq,out] = do_smooth(db_0_0.x_c,db_0_0.y_c,t_db,N_smooth);
    contourf(xq,yq,out,set_of_lines,'LineColor','none','LineWidth',0.1), clearvars t_db xq yq out
    ...
    if i_scale==1, ylabel('y (cm)'); else, set(gca,'ytick',[]); end; set(gca,'xtick',[]);
    title([get_title(set_of_scal(i_scale))]),
    ...
    if i_scale==length(set_of_scal), h = colorbar; ylabel(h,'s (cm)','FontSize',16); set(h,'ytick',[min(set_of_lines) mean(set_of_lines) max(set_of_lines)]); end
end
clearvars N_lines set_of_lines my_colormap h i_scale

% plot DPF
figure(1), N_lines = 15; [set_of_lines,my_colormap] = make_colormap(1,25,N_lines);
for i_scale = 1:length(set_of_scal)
    subplot(4,6,6+i_scale)
...
% plot OD
figure(1), N_lines = 15; [set_of_lines,my_colormap] = make_colormap(0,6,N_lines);
for i_scale = 1:length(set_of_scal)
    subplot(4,6,12+i_scale)
