...
% the analysis: population stat / N
...
% plot N
N_lines = 50; [set_of_lines,my_colormap] = make_colormap(0,3,N_lines);
for i_s = 1:length(set_of_scal)
    figure(6*0+i_s)
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
            ...
            eval(['t_db = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.N);']);
            ...
            set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'N (log_{10}(#))')
...
% plot N - N_{no-tumor}
N_lines = 100; [set_of_lines,my_colormap] = make_colormap(-0.25,+0.25,N_lines);
for i_s = 2:length(set_of_scal)
    ...
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
            ...
            eval(['u_sg = ones(size(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w));']);
            eval(['u_rf = ones(size(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.w));']);
...
% plot N - N_{not-compressed}
N_lines = 30; [set_of_lines,my_colormap] = make_colormap(-0.25,+0.25,N_lines);
for i_s = 3:length(set_of_scal)
    ...
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
            ...
            eval(['u_sg = ones(size(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w));']);
            eval(['u_rf = ones(size(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',1   ),'.','_'),'.w));']);
            ...
            hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_s)) -ry*sqrt(set_of_scal(i_s)) 2*rx*sqrt(set_of_scal(i_s))+eps 2*ry*sqrt(set_of_scal(i_s))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
            hold on, plot(beam_X,beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
            set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'N-N_{not-compress} (log_{10}(#))')
            colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
...
for i_fig = 18:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_',num2str(4),'_fig_',sprintf('%2.0f',i_fig),'.jpg' ],'jpeg'); end
end
