...
% plot I
figure(2), N_lines = 55; [set_of_lines,my_colormap] = make_colormap(8,18,N_lines);
for i_czs = 1:length(set_of_czs)
    cz = set_of_czs(i_czs);
    for i_scale = 1:length(set_of_scal)
        subplot(3,4,(i_scale-1).*length(set_of_czs)+i_czs),
        eval(['t_db = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.w_mean);']);
        contourf(db_1_1_0.x_c,db_1_1_0.y_c,-log(t_db), set_of_lines, 'LineColor','none','LineWidth',0.1), clearvars t_db
...
% plot I - I_{tumor}
i_fig = 3; N_lines = 75; [set_of_lines,my_colormap] = make_colormap(-2.5,+1.5,N_lines);
for i_czs = 1:length(set_of_czs)
    cz = set_of_czs(i_czs);
    for i_scale = 1:length(set_of_scal)
        eval(['x_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.w)/((db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_dpth_photons)+(db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_surf_photons));']);
        eval(['x_rf = (db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.x);']); eval(['y_rf = (db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.y);']); eval(['u_rf = (db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.w)/((db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.no_of_dpth_photons)+(db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.no_of_surf_photons));']);
...
% plot I - I_{cz = 1}
i_fig = 4; N_lines = 75; [set_of_lines,my_colormap] = make_colormap(-0.5,+0.5,N_lines);
for i_czs = 2:length(set_of_czs)
    cz = set_of_czs(i_czs);
    for i_scale = 1:length(set_of_scal)
        eval(['x_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.w)/((db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_dpth_photons)+(db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_surf_photons));']);
        eval(['x_rf = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.x);']); eval(['y_rf = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.y);']); eval(['u_rf = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.w)/((db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.no_of_dpth_photons)+(db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.no_of_surf_photons));']);
        Nx_bin = size(db_0_0_0.N,1); Ny_bin = size(db_0_0_0.N,2);
        [t_db,p_val,x_c,y_c] = do_compare_sum (x_sg,y_sg,u_sg,x_rf,y_rf,u_rf,Lx,Ly,Nx_bin,Ny_bin,@(x)(-log10(x)));
        t_db(p_val>p_threshold) = nan;
        clearvars x_sg y_sg u_sg x_rf y_rf u_rf Nx_bin Ny_bin
        ...
        hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_scale)) -ry*sqrt(set_of_scal(i_scale)) 2*rx*sqrt(set_of_scal(i_scale))+eps 2*ry*sqrt(set_of_scal(i_scale))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
        xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(cz,set_of_scal(i_scale))])
        set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'OD - OD_{cz = 1}(a.u.)')
        colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
        axis(2.*[-2+beam_X/2 +2+beam_X/2 -2+beam_Y/2 +2+beam_Y/2]);
    end
end
...
for i_czs = 1:length(set_of_czs)
    ...
    for i_scale = 2:length(set_of_scal)
        eval(['x_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.w)/((db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_dpth_photons)+(db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_surf_photons));']);
        eval(['x_rf = (db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_rf = (db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_rf = (db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.w)/((db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_dpth_photons)+(db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.no_of_surf_photons));']);
        Nx_bin = size(db_0_0_0.N,1); Ny_bin = size(db_0_0_0.N,2);
        [t_db,p_val,x_c,y_c] = do_compare_sum (x_sg,y_sg,u_sg,x_rf,y_rf,u_rf,Lx,Ly,Nx_bin,Ny_bin,@(x)(-log10(x)));
        t_db(p_val>p_threshold) = nan;
        clearvars x_sg y_sg u_sg x_rf y_rf u_rf Nx_bin Ny_bin
        ...
        hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_scale)) -ry*sqrt(set_of_scal(i_scale)) 2*rx*sqrt(set_of_scal(i_scale))+eps 2*ry*sqrt(set_of_scal(i_scale))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
        xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(cz,set_of_scal(i_scale))])
        set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'OD - OD_{scale = 1}(a.u.)')
        colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
        axis(2.*[-2+beam_X/2 +2+beam_X/2 -2+beam_Y/2 +2+beam_Y/2]);
    end
end
...
for i_fig = 5:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_',num2str(5),'_fig_',sprintf('%1.0f',i_fig),'.jpg' ],'jpeg'); end
end
