...
% the analysis: population stat / s
...
% plot s
figure(2), N_lines = 25; [set_of_lines,my_colormap] = make_colormap(0,50,N_lines);
for i_czs = 1:length(set_of_czs)
    cz = set_of_czs(i_czs);
    for i_scale = 1:length(set_of_scal)
        subplot(3,4,(i_scale-1).*length(set_of_czs)+i_czs),
        eval(['t_db = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.s_mean);']);
        ...
        set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'s (cm)')
...
% % plot s - s_{tumor}
% i_fig = 3; N_lines = 75; [set_of_lines,my_colormap] = make_colormap(-25,50,N_lines);
% for i_czs = 1:length(set_of_czs)
%     cz = set_of_czs(i_czs);
%     for i_scale = 1:length(set_of_scal)
%         eval(['x_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.s);']);
%         eval(['x_rf = (db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.x);']); eval(['y_rf = (db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.y);']); eval(['u_rf = (db_',sprintf('%.0f',0                   ),'_',replace(sprintf('%.1f',0.0              ),'.','_'),'.s);']);
...
%         hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_scale)) -ry*sqrt(set_of_scal(i_scale)) 2*rx*sqrt(set_of_scal(i_scale))+eps 2*ry*sqrt(set_of_scal(i_scale))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
%         xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(cz,set_of_scal(i_scale))])
%         set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'s - s_{no tumor}(cm)')
%         colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
%         axis(2.*[-2+beam_X/2 +2+beam_X/2 -2+beam_Y/2 +2+beam_Y/2]);
%     end
% end
...
% plot s - s_{cz = 1}
i_fig = 4; N_lines = 50; [set_of_lines,my_colormap] = make_colormap(-25,25,N_lines);
for i_czs = 2:length(set_of_czs)
    cz = set_of_czs(i_czs);
    for i_scale = 1:length(set_of_scal)
        eval(['x_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.s);']);
        eval(['x_rf = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.x);']); eval(['y_rf = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.y);']); eval(['u_rf = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(1    )),'.','_'),'.s);']);
        Nx_bin = size(db_0_0_0.N,1); Ny_bin = size(db_0_0_0.N,2);
        [t_db,p_val,x_c,y_c] = do_compare_mean (x_sg,y_sg,u_sg,x_rf,y_rf,u_rf,Lx,Ly,Nx_bin,Ny_bin);
        t_db(p_val>p_threshold) = nan;
        clearvars x_sg y_sg u_sg x_rf y_rf u_rf Nx_bin Ny_bin
        ...
        hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_scale)) -ry*sqrt(set_of_scal(i_scale)) 2*rx*sqrt(set_of_scal(i_scale))+eps 2*ry*sqrt(set_of_scal(i_scale))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
        xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(cz,set_of_scal(i_scale))])
        set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'s - s_{cz = 1}(cm)')
        colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
        axis(2.*[-2+beam_X/2 +2+beam_X/2 -2+beam_Y/2 +2+beam_Y/2]);
    end
end
...
% plot s - s_{scale = 1}
i_fig = 5; N_lines = 50; [set_of_lines,my_colormap] = make_colormap(-50,0,N_lines);
for i_czs = 1:length(set_of_czs)
    cz = set_of_czs(i_czs);
    for i_scale = 2:length(set_of_scal)
        eval(['x_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_sg = (db_',sprintf('%.0f',set_of_scal(i_scale)),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.s);']);
        eval(['x_rf = (db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.x);']); eval(['y_rf = (db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.y);']); eval(['u_rf = (db_',sprintf('%.0f',set_of_scal(1      )),'_',replace(sprintf('%.1f',set_of_czs(i_czs)),'.','_'),'.s);']);
        Nx_bin = size(db_0_0_0.N,1); Ny_bin = size(db_0_0_0.N,2);
        [t_db,p_val,x_c,y_c] = do_compare_mean (x_sg,y_sg,u_sg,x_rf,y_rf,u_rf,Lx,Ly,Nx_bin,Ny_bin);
        t_db(p_val>p_threshold) = nan;
        clearvars x_sg y_sg u_sg x_rf y_rf u_rf Nx_bin Ny_bin
        ...
        hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_scale)) -ry*sqrt(set_of_scal(i_scale)) 2*rx*sqrt(set_of_scal(i_scale))+eps 2*ry*sqrt(set_of_scal(i_scale))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
        xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(cz,set_of_scal(i_scale))])
        set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'s - s_{scale = 1}(cm)')
        colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
        axis(2.*[-2+beam_X/2 +2+beam_X/2 -2+beam_Y/2 +2+beam_Y/2]);
    end
end
...
for i_fig = 5:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_',num2str(1),'_fig_',sprintf('%1.0f',i_fig),'.jpg' ],'jpeg'); end
...
for i_czs = 1:length(set_of_czs)
    ...
    for i_scale = 1:length(set_of_scal)
        % read dbase
        the_filename = ['/home/kaiser/BackUp/Dropbox/research/2.NIRs Project/code/DB/Photon_35_cz_',sprintf('%.1f',cz),'_sclae_',sprintf('%.1f',set_of_scal(i_scale)),'.mat'];
        t_db = load(the_filename); clearvars the_filename
        disp(['cz = ',num2str(cz,'%.1f'),', scale = ',num2str(set_of_scal(i_scale),'%.0f')])
        ...
        % t_db.no_of_photons = t_db.no_of_surf_photons-sum(u~=u_unique(i_fx));
        % clearvars u u_unique freq i_f i_fx

        figure(101)
        [set_of_lines,my_colormap] = make_colormap(0,3,6);
        subplot(3,4,(i_scale-1).*length(set_of_czs)+i_czs),
        contourf(t_db.x_c,t_db.y_c,log10(t_db.N.'),set_of_lines,'LineColor','none','LineWidth',0.1), axis equal, axis tight, colormap("jet"), shading interp, h = colorbar; ylabel(h,'log_{10}(N) (#)'), clim([min(set_of_lines) max(set_of_lines)])
        set(gca,'xtick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), xlabel(['x-axis'])
        set(gca,'ytick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), ylabel(['y-axis'])
        title(['cz = ',num2str(set_of_czs(i_czs),'%.1f'),' cm, scale = ',num2str(set_of_scal(i_scale),'%.0f')])

        figure(102)
        [set_of_lines,my_colormap] = make_colormap(0,100,20);
        subplot(3,4,(i_scale-1).*length(set_of_czs)+i_czs),
        contourf(t_db.x_c,t_db.y_c,(t_db.s_mean.'),set_of_lines,'LineColor','none','LineWidth',0.1), axis equal, axis tight, colormap("jet"), shading interp, h = colorbar; ylabel(h,'s (cm)'), clim([min(set_of_lines) max(set_of_lines)])
        set(gca,'xtick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), xlabel(['x-axis'])
        set(gca,'ytick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), ylabel(['y-axis'])
        title(['cz = ',num2str(set_of_czs(i_czs),'%.1f'),' cm, scale = ',num2str(set_of_scal(i_scale),'%.0f')])

        figure(103)
        [set_of_lines,my_colormap] = make_colormap(0,50,10);
        subplot(3,4,(i_scale-1).*length(set_of_czs)+i_czs),
        contourf(t_db.x_c,t_db.y_c,(t_db.s_mean.')./(t_db.d_mean.'),set_of_lines,'LineColor','none','LineWidth',0.1), axis equal, axis tight, colormap("jet"), shading interp, h = colorbar; ylabel(h,'s (cm)'), clim([min(set_of_lines) max(set_of_lines)])
        set(gca,'xtick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), xlabel(['x-axis'])
        set(gca,'ytick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), ylabel(['y-axis'])
        title(['cz = ',num2str(set_of_czs(i_czs),'%.1f'),' cm, scale = ',num2str(set_of_scal(i_scale),'%.0f')])

        figure(104)
        [set_of_lines,my_colormap] = make_colormap(3,9,12);
        subplot(3,4,(i_scale-1).*length(set_of_czs)+i_czs),
        contourf(t_db.x_c,t_db.y_c,-log10(t_db.w_mean.'),set_of_lines,'LineColor','none','LineWidth',0.1), axis equal, axis tight, colormap("jet"), shading interp, h = colorbar; ylabel(h,'s (cm)'), % clim([min(set_of_lines) max(set_of_lines)])
        set(gca,'xtick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), xlabel(['x-axis'])
        set(gca,'ytick',[round(min(t_db.x_c),1) 0 round(max(t_db.x_c),1)]), ylabel(['y-axis'])
        title(['cz = ',num2str(set_of_czs(i_czs),'%.1f'),' cm, scale = ',num2str(set_of_scal(i_scale),'%.0f')])
