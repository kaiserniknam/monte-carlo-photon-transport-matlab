...
% the analysis: population stat / OD for Dr. Das grant proposal
...
% plot OD
N_lines = 55; [set_of_lines,my_colormap] = make_colormap(2,18,N_lines);
for i_B = 2:length(set_of_beam_X)
    figure(i_B-1)
    beam_X = set_of_beam_X(i_B);
    beam_Y = set_of_beam_Y(i_B);
    for i_s = 1:length(set_of_scal)
        scal = set_of_scal(i_s);
        subplot(2,3,i_s)
        eval(['t_db = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w_mean);']);
        contourf(db_0_0_0_0_0_0.x_c,db_0_0_0_0_0_0.y_c,-log(t_db(1:end,1:end)+t_db(1:end,end:-1:1)+t_db(end:-1:1,1:end)+t_db(end:-1:1,end:-1:1)).',set_of_lines,'LineColor','none','LineWidth',0.1), clearvars t_db
        ...
        title(get_title(scal));
        xlabel('x (cm)');
        set(gca,'ytick',[-3   +3]), ylim([-3 +3])
        set(gca,'ytick',[-3:3:+3]), ylim([-3 +3])
        ylabel('y (cm)');
        set(gca,'xtick',[-3   +3]), xlim([-3 +3])
        set(gca,'xtick',[-3:3:+3]), xlim([-3 +3])

        clearvars t_db h scal
    end
    clearvars i_s h beam_X beam_Y
end
clearvars N_lines set_of_lines my_colormap i_czs i_scale i_B



% plot OD - OD_{no-tumor}
N_lines = 75; [set_of_lines,my_colormap] = make_colormap(-0.5,+1.5,N_lines);
for i_B = 2:length(set_of_beam_X)
    figure(i_B-1+3)
    beam_X = set_of_beam_X(i_B);
    beam_Y = set_of_beam_Y(i_B);
    for i_s = 2:length(set_of_scal)
        ...
        subplot(2,3,i_s)
        eval(['t_db = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w_mean);']);
        eval(['t_rf = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.w_mean);']);

        contourf(db_0_0_0_0_0_0.x_c,db_0_0_0_0_0_0.y_c, ...
            -log(t_db(1:end,1:end)+t_db(1:end,end:-1:1)+t_db(end:-1:1,1:end)+t_db(end:-1:1,end:-1:1)).' ...
            +log(t_rf(1:end,1:end)+t_rf(1:end,end:-1:1)+t_rf(end:-1:1,1:end)+t_rf(end:-1:1,end:-1:1)).' ...
            ,set_of_lines,'LineColor','none','LineWidth',0.1), clearvars t_db
        hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_s)) -ry*sqrt(set_of_scal(i_s)) 2*rx*sqrt(set_of_scal(i_s))+eps 2*ry*sqrt(set_of_scal(i_s))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
        hold on, plot(+beam_X,+beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        hold on, plot(+beam_X,-beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        hold on, plot(-beam_X,+beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        hold on, plot(-beam_X,-beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'OD-OD_{no-tumor} (a.u.)')
        colormap('jet'); clim([min(set_of_lines) max(set_of_lines)])
...
% plot OD - OD_{not-compressed}
N_lines = 125; [set_of_lines,my_colormap] = make_colormap(-0.25,+0.25,N_lines);
for i_B = 2:length(set_of_beam_X)
    figure(i_B-1+6)
    beam_X = set_of_beam_X(i_B);
    beam_Y = set_of_beam_Y(i_B);
    for i_s = 3:length(set_of_scal)
        ...
        hold on, plot(-beam_X,+beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        hold on, plot(-beam_X,-beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'OD-OD_{OD-OD_{not-compress} (a.u.)} (a.u.)')
        colormap('jet'); % clim([min(set_of_lines) max(set_of_lines)])
...
for i_B = 2:length(set_of_beam_X)
    ...
    for i_s = 2:length(set_of_scal)
        ...
        eval(['u_sg = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w)./((db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_dpth_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_surf_photons));']);
        eval(['u_rf = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.w)./((db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.no_of_dpth_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.no_of_surf_photons));']);

        Nx_bin = size(db_0_0_0_0_0_0.N,1); Ny_bin = size(db_0_0_0_0_0_0.N,2);
        [t_db,p_val,x_c,y_c] = do_compare_sum (x_sg,y_sg,u_sg,x_rf,y_rf,u_rf,Lx,Ly,Nx_bin,Ny_bin,@(x)(-log(x)));
        t_db(p_val>p_threshold) = nan;
        clearvars x_sg y_sg u_sg x_rf y_rf u_rf Nx_bin Ny_bin

        contourf(x_c, y_c, (t_db(1:end,1:end)+t_db(1:end,end:-1:1)+t_db(end:-1:1,1:end)+t_db(end:-1:1,end:-1:1)).', 'LineColor', 'none', 'LineWidth', 0.1), clearvars t_db p_val x_c y_c
        hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_s)) -ry*sqrt(set_of_scal(i_s)) 2*rx*sqrt(set_of_scal(i_s))+eps 2*ry*sqrt(set_of_scal(i_s))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
        hold on, plot(+beam_X,+beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        hold on, plot(+beam_X,-beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
        hold on, plot(-beam_X,+beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
...
for i_B = 2:length(set_of_beam_X)
    ...
    for i_s = 3:length(set_of_scal)
        ...
        eval(['u_sg = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w)./((db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_dpth_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_surf_photons));']);
        eval(['u_rf = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',1   ),'.','_'),'.w)./((db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.no_of_dpth_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.no_of_surf_photons));']);
...
for i_fig = 15:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_',num2str(5),'_fig_',sprintf('%2.0f',i_fig),'.jpg' ],'jpeg'); end
end
