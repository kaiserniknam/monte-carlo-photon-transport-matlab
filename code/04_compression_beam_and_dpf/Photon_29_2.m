...
for i_scale = 1:length(set_of_scal)
    ...
    eval(['rf_',replace(sprintf('%.1f',set_of_scal(i_scale)),'.','_'),' = load(''/home/kaiser/BackUp/Dropbox/research/2.NIRs Project/code/DB/Photon_',num2str(31),      '_X_',num2str(0),     '_Y_',num2str(0),     '_phi_',num2str(beam_phi),'_tht_',num2str(beam_phi),'_sclae_',sprintf('%.1f',set_of_scal(i_scale)),'.mat'');'])
end
...
for i_scale = 1:length(set_of_scal)
    ...
    ylabel(h,'s_{off} - s (cm)','FontSize',16)
end
...
for i_scale = 1:length(set_of_scal)
    ...
    xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(set_of_scal(i_scale))]),
    set(gca,'fontsize',16), axis equal, axis tight, grid on; h = colorbar; axis(2.*[-1+beam_X/2 +1+beam_X/2 -1+beam_Y/2 +1+beam_Y/2]); colormap("jet"); clim([min(set_of_lines) max(set_of_lines)])
    ylabel(h,'DPF_{off} - DPF','FontSize',16)
end
...
for i_scale = 1:length(set_of_scal)
    ...
    xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(set_of_scal(i_scale))]),
    set(gca,'fontsize',16), axis equal, axis tight, grid on; h = colorbar; axis(2.*[-1+beam_X/2 +1+beam_X/2 -1+beam_Y/2 +1+beam_Y/2]); colormap("jet"); clim([min(set_of_lines) max(set_of_lines)])
    ylabel(h,'OD_{off} - OD','FontSize',16)
end
clearvars N_lines set_of_lines my_colormap h i_scale idx
