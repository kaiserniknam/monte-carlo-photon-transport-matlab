...
% For Arnab research / SPIE WEST 2025

clc
close all

z_air = 0.0; % the thickness of air layer
wvlnt = 850; % sample wavelength (nm)
nPhotonsReq = 1000;  % number of requested photon
nExamplePaths = 1000; % number of example photon paths %%%%%%%%%%%%%%%%%%%%
% Optical properties of breast and tumor [1-4]
[mua_brst,~,mus_brst,~,brst_strct,~] = give_breast_mua_mus(wvlnt,false);
...
idx = 1; g_brst = brst_strct(idx).g; n_brst = brst_strct(idx).n; mus_brst = mus_brst/(1-g_brst);
[st, x, y, z, d, s, w, a, no_of_surf_photons, no_of_dpth_photons, M_raw] = do_simulation ( ...
    [mua_brst,mus_brst,g_brst,n_brst], ...
    [nan     ,nan     ,nan   ,nan   ], ...
    [Lx,Ly,Lz],[nan,nan,nan], ...
    [nan,nan,nan],z_air,wvlnt,nPhotonsReq,nExamplePaths,beam_X,beam_Y,beam_phi,beam_tht);
...
% model = plot(model,'G');
clearvars opt_bckg opt_sgnl dl
...
while size(scatter_stat,1) < nExamplePaths
    ...
    TheST(1) = -cmd_size(1)/2 + cmd_size(1)*rand;
    TheST(2) = -cmd_size(2)/2 + cmd_size(2)*rand;
    TheST(3) =              0 + cmd_size(3)*rand;
