...
% the analysis: population stat / OD
...
% plot OD
N_lines = 55; [set_of_lines,my_colormap] = make_colormap(8,18,N_lines);
for i_s = 1:length(set_of_scal)
    figure(6*0+i_s)
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
            ...
            contourf(db_0_0_0_0_0_0.x_c,db_0_0_0_0_0_0.y_c,-log(t_db).',set_of_lines,'LineColor','none','LineWidth',0.1), clearvars t_db
...
N_lines = 75; [set_of_lines,my_colormap] = make_colormap(-2.5,+1.5,N_lines);
...
for i_s = 3:length(set_of_scal)
    ...
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
            ...
            eval(['u_sg = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w)./((db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_dpth_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_surf_photons));']);
            eval(['u_rf = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',1   ),'.','_'),'.w)./((db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',1   ),'.','_'),'.no_of_dpth_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',1   ),'.','_'),'.no_of_surf_photons));']);
            ...
            hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_s)) -ry*sqrt(set_of_scal(i_s)) 2*rx*sqrt(set_of_scal(i_s))+eps 2*ry*sqrt(set_of_scal(i_s))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
            hold on, plot(beam_X,beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
            set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'OD-OD_{not-compress} (a.u.)')
            colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
...
for i_fig = 18:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_',num2str(5),'_fig_',sprintf('%2.0f',i_fig),'.jpg' ],'jpeg'); end
end
