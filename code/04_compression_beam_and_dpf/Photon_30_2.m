...
for i_scale = 1:length(set_of_scal)
    ...
    eval(['rf_',replace(sprintf('%.1f',set_of_scal(i_scale)),'.','_'),' = load(''/home/kaiser/BackUp/Dropbox/research/2.NIRs Project/code/DB/Photon_',num2str(29),      '_X_',num2str(beam_X),'_Y_',num2str(beam_Y),'_phi_',num2str(0),       '_tht_',num2str(0),       '_sclae_',sprintf('%.1f',set_of_scal(i_scale)),'.mat'');'])
end
...
for i_scale = 1:length(set_of_scal)
    ...
    ylabel(h,'s_{off,tilted} - s_{off} (cm)','FontSize',16)
end
...
for i_scale = 1:length(set_of_scal)
    ...
    xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(set_of_scal(i_scale))]),
    set(gca,'fontsize',16), axis equal, axis tight, grid on; h = colorbar; axis(2.*[-1+beam_X/2 +1+beam_X/2 -1+beam_Y/2 +1+beam_Y/2]); colormap("jet"); clim([min(set_of_lines) max(set_of_lines)])
    ylabel(h,'DPF_{off,tilted} - DPF_{off}','FontSize',16)
end
...
for i_scale = 1:length(set_of_scal)
    ...
    xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(set_of_scal(i_scale))]),
    set(gca,'fontsize',16), axis equal, axis tight, grid on; h = colorbar; axis(2.*[-1+beam_X/2 +1+beam_X/2 -1+beam_Y/2 +1+beam_Y/2]); colormap("jet"); clim([min(set_of_lines) max(set_of_lines)])
    ylabel(h,'OD_{off,tilted} - OD_{off}','FontSize',16)
end
clearvars N_lines set_of_lines my_colormap h i_scale idx
...
for i_scale = 1:length(set_of_scal)
    ...
    xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(set_of_scal(i_scale))]),
    set(gca,'fontsize',16), axis equal, axis tight, grid on; h = colorbar; axis(2.*[-1+beam_X/2 +1+beam_X/2 -1+beam_Y/2 +1+beam_Y/2]); colormap("jet"); clim([min(set_of_lines) max(set_of_lines)])
    ylabel(h,'N_{off,tilted} - N_{off} (cm)','FontSize',16)
end
clearvars N_lines set_of_lines my_colormap h i_scale idx
