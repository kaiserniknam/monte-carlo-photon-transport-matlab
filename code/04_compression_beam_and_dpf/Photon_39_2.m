...
% the analysis: population stat / DPF
...
% plot DPF
N_lines = 25; [set_of_lines,my_colormap] = make_colormap(0,25,N_lines);
for i_s = 1:length(set_of_scal)
    figure(6*0+i_s)
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
...
% plot DPF - DPF_{no-tumor}
N_lines = 25; [set_of_lines,my_colormap] = make_colormap(-1.0,+1.0,N_lines);
for i_s = 2:length(set_of_scal)
    ...
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
            ...
            eval(['n_sg = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_surf_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_dpth_photons);']);
            eval(['n_rf = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.no_of_surf_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',0   ),'.','_'),'.no_of_dpth_photons);']);
            eval(['u_sg = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w);']);
            ...
            hold on, rectangle('Position',[-rx*sqrt(set_of_scal(i_s)) -ry*sqrt(set_of_scal(i_s)) 2*rx*sqrt(set_of_scal(i_s))+eps 2*ry*sqrt(set_of_scal(i_s))+eps],'Curvature',[1 1],'LineWidth',1,'LineStyle','-.'), hold off % breast area
            hold on, plot(beam_X,beam_Y,'o','LineWidth',1,'MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','y'), hold off
            set(gca,'fontsize',12), axis equal, axis tight, grid on; h = colorbar; ylabel(h,'OD/d-OD/d_{no-tumor} (cm^{-1})')
            colormap(my_colormap); clim([min(set_of_lines) max(set_of_lines)])
...
for i_s = 3:length(set_of_scal)
    ...
    for i_X = 1:length(set_of_beam_X)
        for i_Y = 1:length(set_of_beam_Y)
            ...
            eval(['n_sg = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_surf_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.no_of_dpth_photons);']);
            eval(['n_rf = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',1   ),'.','_'),'.no_of_surf_photons)+(db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',1   ),'.','_'),'.no_of_dpth_photons);']);
            eval(['u_sg = (db_',replace(sprintf('%.1f',beam_X),'.','_'),'_',replace(sprintf('%.1f',beam_Y),'.','_'),'_',replace(sprintf('%.1f',scal),'.','_'),'.w);']);
...
for i_fig = 18:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_',num2str(2),'_fig_',sprintf('%2.0f',i_fig),'.jpg' ],'jpeg'); end
end
