...
nPhotonsReq = 10000;  % number of requested photon
...
for i_czs = 1:length(set_of_czs)
    ...
    for i_scale = 1:length(set_of_scal)
        ...
        mua_tumr = tumr_strct(idx).mua; g_tumr = tumr_strct(idx).g; n_tumr = tumr_strct(idx).n; mus_tumr = tumr_strct(idx).mus/(1-g_tumr); wvlnt = tumr_strct(idx).lambda; clearvars idx
        [st, x, y, z, d, s, w, a, no_of_surf_photons, no_of_dpth_photons, M_raw] = do_simulation ( ...
        ...
        save  (['Photon_35_cz_',sprintf('%.1f',cz),'_sclae_',sprintf('%.1f',set_of_scal(i_scale)),'.mat' ], ...
            'st', 'x', 'y', 'z', 'd', 's', 'w', 'a', 'no_of_surf_photons', 'no_of_dpth_photons', 'M_raw', ...
            'w_mean','d_mean','s_mean','s_var','N','N_samples','x_c','y_c','cx','cy','cz','rx','ry','rz')
        clearvars ind_x ind_y i_sample N_samples

        clearvars st x y z d s w a x_c y_c w_mean d_mean s_mean s_var no_of_dpth_photons no_of_surf_photons N N_bin M_raw wvlnt
...
function [st, x, y, z, d, s, w, a, no_of_surf_photons, no_of_dpth_photons, M_raw] = do_simulation (opt_bckg,opt_sgnl,cmd_size,sig_pos,sig_size,zSurface,wvlngth,nPhotonsReq,nExamplePaths,beam_X,beam_Y,beam_phi,beam_tht)
%% Geometry definition
...
while size(scatter_stat,1) < nExamplePaths
    t_model = model;
    % select a random point inside tumor
    center = sig_pos;
    semiAxes = sig_size;
    % generate the random point in spherical coordinates
    TheTheta = 2*pi*rand;
    ThePhi = acos(2*rand-1);
    TheR = rand^(1/3);
    % convert to Cartesian coordinates
    TheX = TheR*cos(TheTheta)*sin(ThePhi);
    TheY = TheR*sin(TheTheta)*sin(ThePhi);
    TheZ = TheR*cos(ThePhi);
    TheST = center + [TheX*semiAxes(1), TheY*semiAxes(2), TheZ*semiAxes(3)];
    t_model.MC.lightSource.xFocus = TheST(1); % [cm] x position of focus
    t_model.MC.lightSource.yFocus = TheST(2); % [cm] y position of focus
    t_model.MC.lightSource.zFocus = TheST(3); % [cm] z position of focus
    M_raw = t_model.G.M_raw;
    clearvars center semiAxes TheTheta ThePhi TheR TheX TheY TheZ TheST
    ...
    t_scatter_stat = nan(length(fnsh_pnts),3+1+1+3+3); clearvars spratrs
        ...
        if isnan(sum(sum(Photon_Path)))
            ...
            if Photon_Path(3,end) <= 0.0
                ...
                t_scatter_stat(idx,:) = [...
                    ...
                    (t_model.MC.lightSource.xFocus), ...
                    (t_model.MC.lightSource.yFocus), ...
                    (t_model.MC.lightSource.zFocus), ...
                    ];
            else
...
st= scatter_stat(:,9:11);                                              % st-> initial points
end
