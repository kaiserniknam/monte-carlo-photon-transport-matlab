function [] = Photon_31_graphic ()
% Repository group: 04_compression_beam_and_dpf
% Version role: visualization
%
% Research note: This file is retained as a documented historical version.
% Review configuration, external data dependencies, and output paths before use.
% Impact of Tumor/Signal on DPF/PPF Series Simulations
% this version: s vs. d for breast tissue (compression-induced hemodynamic changes in breast tissue: beam angle, 2-D measurement -> beam at center)
...
nExamplePaths = 100; % number of example photon paths
...
cubeHalfSize_z = Lz/2;
% Define the sphere
[x_sphere, y_sphere, z_sphere] = sphere(50); % Generate sphere coordinates
% Scale the sphere to fit inside the cube
x_sphere = rx * x_sphere;
y_sphere = ry * y_sphere;
z_sphere = rz * z_sphere;
% Plot the cube
hold on; axis equal;
h_z = 1;
% Cube vertices
vertices = [
    -cubeHalfSize_x -cubeHalfSize_y  0-h_z;
     cubeHalfSize_x -cubeHalfSize_y  0-h_z;
     cubeHalfSize_x  cubeHalfSize_y  0-h_z;
    -cubeHalfSize_x  cubeHalfSize_y  0-h_z;
    -cubeHalfSize_x -cubeHalfSize_y  -2*cubeHalfSize_z-h_z;
     cubeHalfSize_x -cubeHalfSize_y  -2*cubeHalfSize_z-h_z;
     cubeHalfSize_x  cubeHalfSize_y  -2*cubeHalfSize_z-h_z;
    -cubeHalfSize_x  cubeHalfSize_y  -2*cubeHalfSize_z-h_z;
];
...
patch('Vertices', vertices, 'Faces', faces, ...
      'FaceColor', 'b', 'EdgeColor', 'black', 'LineWidth', 1,'FaceAlpha', 0.1);
% Draw the sphere
surf(x_sphere, y_sphere, z_sphere-cz-h_z, 'FaceAlpha', 1.0, 'EdgeColor', 'none', 'FaceColor', 'y');
% Labels and view
view(3); grid on; hold off; axis equal, axis off
clearvars x_sphere y_sphere z_sphere cubeHalfSize_x cubeHalfSize_y cubeHalfSize_z faces
hold on
...
N_lines = 15; set_of_lines = linspace(0,14,2*N_lines+1);
eval(['t_db = (db_',strrep(sprintf('%.1f',set_of_scal(i_scale)),'.','_'),'.s_mean);']);
[xq,yq,out] = do_smooth(db_0_0.x_c,db_0_0.y_c,t_db,N_smooth);
contourf(xq,yq,out,set_of_lines,'LineColor','none','LineWidth',0.1), clearvars t_db xq yq out code_num db_0_0 db_1_0 db_2_0 db_3_0 db_4_0 db_5_0 db_2_5 db_1_5  i_scale N_lines N_smooth
colormap jet, view([-45 25])
...
for idx = 1
    ...
    for i_scale = 1:length(set_of_scal)
        ...
        figure(17), subplot(2,5,i_scale)
        imagesc(linspace(-Lx/2,+Lx/2,size(M_raw,1)),linspace(-Ly/2,+Ly/2,size(M_raw,2)),squeeze(M_raw(:,:,round(cz/set_of_scal(i_scale)/0.03))).')
        xlabel('x (cm)'), ylabel('y (cm)'), title([get_title(set_of_scal(i_scale))]),
        set(gca,'fontsize',12), axis equal, axis tight, axis(2.*[-1 +1 -1 +1]);
        figure(17), subplot(2,5,5+i_scale)
        imagesc(linspace(-Lx/2,+Lx/2,size(M_raw,1)),linspace(0,Lz,size(M_raw,3)),squeeze(M_raw(:,round(Ly/2/0.03),:)).')
