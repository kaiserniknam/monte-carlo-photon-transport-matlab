...
% this version: s vs. d for breast tissue (over mus)
...
[~,~,~,~,brst_strct,~] = give_breast_mua_mus(wvlnt_sample,true);
Lx = 19.1; Ly = 19.1; Lz = 10.0; % The size of the computational domain (in cm) to prevent reflections from the boundaries.

for idx = 1:1
    do_count = 0;
    set_of_mus = linspace(brst_strct(idx).mus*0.5,brst_strct(idx).mus*1.5,7);
    for i_mus = 1:length(set_of_mus)
        do_count = do_count + 1;
        mua_brst = brst_strct(idx).mua; g_brst = brst_strct(idx).g; n_brst = brst_strct(idx).n; mus_brst = set_of_mus(i_mus)/(1-g_brst); wvlnt = brst_strct(idx).lambda;
        ...
        plot(d,s,'o','MarkerFaceColor',get_color(do_count),'MarkerEdgeColor','k','DisplayName',['\mu_{s} = ',num2str(mus_brst),' cm^{-1}, \mu_{a} = ',num2str(mua_brst),' cm^{-1}'])
        xlabel('d (cm)'), ylabel('s (cm)'), title('d vs. s')
        set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

        figure(2), errorbar(d_c,s_mean,s_var,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst),' cm^{-1}, \mu_{a} = ',num2str(mua_brst),' cm^{-1}'],'Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
        set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

        figure(3), plot(d_c,s_mean,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst),' cm^{-1}, \mu_{a} = ',num2str(mua_brst),' cm^{-1}'],'Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
        set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

        figure(4), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst),' cm^{-1}, \mu_{a} = ',num2str(mua_brst),' cm^{-1}'],'Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s/d (cm)'), title('~ DPF')
        set(gca,'fontsize',24), axis tight, grid on, legend show, hold on

        figure(5), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst),' cm^{-1}, \mu_{a,b} = ',num2str(mua_brst),' cm^{-1}'],'Color',get_color(do_count)), hold on
        musp_brst = mus_brst*(1-g_brst);
        figure(5), plot(d_c,(sqrt(3*musp_brst)/2/sqrt(mua_brst)).*(d_c.*sqrt(3*musp_brst*mua_brst))./(d_c.*sqrt(3*musp_brst*mua_brst)+1),'LineWidth',2,'LineStyle','-.','Color',get_color(do_count),'DisplayName',['analytic. \mu_{s} = ',num2str(mus_brst),' cm^{-1}, \mu_{a} = ',num2str(mua_brst),'  cm^{-1}']), hold on
        xlabel('d (cm)'), ylabel('s/d (cm)'), title('~ DPF')
        set(gca,'fontsize',24), axis tight, grid on, legend show, hold on
