...
% this version: s vs. d for breast tissue (over wavelength)
...
set_of_lambda = linspace(650,1050,5);

do_count = 0;
for i_lambda = 1:length(set_of_lambda)
    do_count = do_count + 1;
    % Optical properties of breast and tumor [1-4]
    [mua_brst,~,mus_brst,~,brst_strct,~] = give_breast_mua_mus(set_of_lambda(i_lambda),false);
    idx = 1; g_brst = brst_strct(idx).g; n_brst = brst_strct(idx).n; mus_brst = mus_brst/(1-g_brst); wvlnt = set_of_lambda(i_lambda);
    [d, s, w, a, no_of_surf_photons, no_of_dpth_photons] = do_simulation ( ...
        [mua_brst,mus_brst,g_brst,n_brst], ...
        [nan     ,nan     ,nan   ,nan   ], ...
        [Lx,Ly,Lz],[nan,nan,nan],[nan,nan,nan],z_air,wvlnt,nPhotonsReq,nExamplePaths);
    ...
    plot(d,s,'o','MarkerFaceColor',get_color(do_count),'MarkerEdgeColor','k','DisplayName',['\mu_{s} = ',num2str(mus_brst,'%.0f'),' cm^{-1}, \mu_{a} = ',num2str(mua_brst,'%.2f'),' cm^{-1}, \lambda = ',num2str(set_of_lambda(i_lambda)),' nm'])
    ...
    figure(2), errorbar(d_c,s_mean,s_var,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst,'%.0f'),' cm^{-1}, \mu_{a} = ',num2str(mua_brst,'%.2f'),' cm^{-1}, \lambda = ',num2str(set_of_lambda(i_lambda)),' nm'],'Color',get_color(do_count)), hold on
    xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
    set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

    figure(3), plot(d_c,s_mean,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst,'%.0f'),' cm^{-1}, \mu_{a} = ',num2str(mua_brst,'%.2f'),' cm^{-1}, \lambda = ',num2str(set_of_lambda(i_lambda)),' nm'],'Color',get_color(do_count)), hold on
    xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
    set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

    figure(4), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst,'%.0f'),' cm^{-1}, \mu_{a} = ',num2str(mua_brst,'%.2f'),' cm^{-1}, \lambda = ',num2str(set_of_lambda(i_lambda)),' nm'],'Color',get_color(do_count)), hold on
    xlabel('d (cm)'), ylabel('s/d (cm)'), title('~ DPF')
    set(gca,'fontsize',24), axis tight, grid on, legend show, hold on

    figure(5), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['\mu_{s} = ',num2str(mus_brst,'%.0f'),' cm^{-1}, \mu_{a} = ',num2str(mua_brst,'%.2f'),' cm^{-1}, \lambda = ',num2str(set_of_lambda(i_lambda)),' nm'],'Color',get_color(do_count)), hold on
    musp_brst = mus_brst*(1-g_brst);
    figure(5), plot(d_c,(sqrt(3*musp_brst)/2/sqrt(mua_brst)).*(d_c.*sqrt(3*musp_brst*mua_brst))./(d_c.*sqrt(3*musp_brst*mua_brst)+1),'LineWidth',2,'LineStyle','-.','Color',get_color(do_count),'DisplayName',['analytic. \mu_{s} = ',num2str(mus_brst,'%.0f'),' cm^{-1}, \mu_{a} = ',num2str(mua_brst,'%.2f'),' cm^{-1}, \lambda = ',num2str(set_of_lambda(i_lambda)),' nm']), hold on
    xlabel('d (cm)'), ylabel('s/d (cm)'), title('~ DPF')
    set(gca,'fontsize',24), axis tight, grid on, legend show, hold on
