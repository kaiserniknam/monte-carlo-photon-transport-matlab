...
% this version: s vs. d for breast tissue (over tumor depth)
...
for idx = 1
    set_of_dpth = 1:0.5:3; % set of depth
    for i_dpth = 1:length(set_of_dpth)
        do_count = do_count + 1;
        ...
        [x, y, d, s, w, a, no_of_surf_photons, no_of_dpth_photons] = do_simulation ( ...
            [mua_brst,mus_brst,g_brst,n_brst], ...
            [mua_tumr,mus_tumr,g_tumr,n_tumr], ...
            [Lx,Ly,Lz],[cx,cy,set_of_dpth(i_dpth)],[rx,ry,rz],z_air,wvlnt,nPhotonsReq,nExamplePaths);
        ...
        plot(d,s,'o','MarkerFaceColor',get_color(do_count),'MarkerEdgeColor','k','DisplayName',['depth = ',num2str(set_of_dpth(i_dpth)),' cm'])
        ...
        figure(2), errorbar(d_c,s_mean,s_var,'LineWidth',2,'DisplayName',['\mu_{s,b} = ',num2str(mus_brst),' cm^{-1}, \mu_{a,b} = ',num2str(mua_brst),' cm^{-1}, \mu_{s,t} = ',num2str(mus_tumr),' cm^{-1}, \mu_{a,t} = ',num2str(mua_tumr),' cm^{-1}, depth = ',num2str(set_of_dpth(i_dpth)),' cm'],'Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
        set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

        figure(3), plot(d_c,s_mean,'LineWidth',2,'DisplayName',['depth = ',num2str(set_of_dpth(i_dpth)),' cm'],'Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s (cm)'), title('s vs. d')
        set(gca,'fontsize',24), axis tight, grid on, legend('show','Location','northwest'), hold on

        figure(4), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['depth = ',num2str(set_of_dpth(i_dpth)),' cm'],'Color',get_color(do_count)), hold on
        xlabel('d (cm)'), ylabel('s/d (cm)'), title('~ DPF')
        set(gca,'fontsize',24), axis tight, grid on, legend show, hold on

        figure(5)
        [N_s,edges_s] = histcounts(s,0:.5:100);
        loglog(1/2*(edges_s(1:end-1)+edges_s(2:end-0)),N_s,'Color',get_color(do_count),'DisplayName',['depth = ',num2str(set_of_dpth(i_dpth)),' cm']), hold on
        xlabel('s (cm)'), ylabel('freq (a.u.)'), title('hist. of s')
        set(gca,'fontsize',24), axis tight, grid on, legend show, hold on
        clearvars N_s edges_s

       save  (['Photon_22_scale_',sprintf('%.1f',set_of_dpth(i_dpth)),'.mat' ], ...
            'x', 'y', 'd', 's', 'w', 'a', 'no_of_surf_photons', 'no_of_dpth_photons', ...
            'w_mean','s_mean','s_var','d_c')
...
function [x , y, d, s, w, a, no_of_surf_photons, no_of_dpth_photons] = do_simulation (opt_bckg,opt_sgnl,cmd_size,sig_pos,sig_size,zSurface,wvlngth,nPhotonsReq,nExamplePaths)
%% Geometry definition
