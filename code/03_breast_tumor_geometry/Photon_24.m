...
% this version: s vs. d for breast tissue (Experiencing Reflection at Boundaries)
...
[~,~,~,~,brst_strct,~] = give_breast_mua_mus(wvlnt_sample,false);
Lx   = 13.1; Ly = 13.1; Lz = 10.0; % The size of the computational domain (in cm)
do_count = 0;

set_of_thickness = 1:1:6;
for i_thickness = 1:length(set_of_thickness)
    do_count = do_count + 1;
    idx = 1;
    mua_brst = brst_strct(idx).mua; g_brst = brst_strct(idx).g; n_brst = brst_strct(idx).n; mus_brst = brst_strct(idx).mus/(1-g_brst); wvlnt = brst_strct(idx).lambda; clearvars idx
    [d, s, w, a, no_of_surf_photons, no_of_dpth_photons] = do_simulation ( ...
        [mua_brst,mus_brst,g_brst,n_brst], ...
        [nan,     nan,     nan,   nan   ], ...
        [Lx,Ly,Lz],[nan,nan,nan],[nan,nan,nan],z_air,wvlnt,nPhotonsReq,nExamplePaths,set_of_thickness(i_thickness));
    ...
    plot(d,s,'o','MarkerFaceColor',get_color(do_count),'MarkerEdgeColor','k','DisplayName',['size = ',num2str(Lx-2*set_of_thickness(i_thickness)),', X ',num2str(num2str(Ly-2*set_of_thickness(i_thickness))),' X ',num2str(num2str(Lz)),' cm^3'])
    ...
    figure(2), plot(s_f,-log10(w_f),'o','MarkerFaceColor',get_color(do_count),'MarkerEdgeColor','k','DisplayName',['size = ',num2str(Lx-2*set_of_thickness(i_thickness)),', X ',num2str(num2str(Ly-2*set_of_thickness(i_thickness))),' X ',num2str(num2str(Lz)),' cm^3']), hold on
    ...
    figure(3), plot(d_c,-log10(w_mean),'LineWidth',2,'DisplayName',['size = ',num2str(Lx-2*set_of_thickness(i_thickness)),', X ',num2str(num2str(Ly-2*set_of_thickness(i_thickness))),' X ',num2str(num2str(Lz)),' cm^3'],'Color',get_color(do_count)), hold on
    ...
    figure(4), plot(d_c,s_mean,'LineWidth',2,'DisplayName',['size = ',num2str(Lx-2*set_of_thickness(i_thickness)),', X ',num2str(num2str(Ly-2*set_of_thickness(i_thickness))),' X ',num2str(num2str(Lz)),' cm^3'],'Color',get_color(do_count)), hold on
    if isscalar(mua_brst) && isscalar(mus_brst)
    ...
    figure(5), plot(d_c,s_mean./d_c,'LineWidth',2,'DisplayName',['size = ',num2str(Lx-2*set_of_thickness(i_thickness)),', X ',num2str(num2str(Ly-2*set_of_thickness(i_thickness))),' X ',num2str(num2str(Lz)),' cm^3'],'Color',get_color(do_count)), hold on
...
function [d, s, w, a, no_of_surf_photons, no_of_dpth_photons] = do_simulation (opt_bckg,opt_sgnl,cmd_size,sig_pos,sig_size,zSurface,wvlngth,nPhotonsReq,nExamplePaths,thck)
%% Geometry definition
...
model.G.geomFuncParams      = {zSurface,2,3,sig_pos(1),sig_pos(2),sig_pos(3),sig_size(1),sig_size(2),sig_size(3),thck}; % A cell array that you can use to contain all sorts of inputs you would like to use inside the geomFunc
% model = plot(model,'G');
clearvars opt_bckg opt_sgnl cmd_size sig_pos sig_size
    ...
    for idx = 1:length(strt_pnts)
        ...
        if isnan(sum(sum(Photon_Path)))
            ...
            if Photon_Path(3,end) <= 0.0
                if sum(abs(Photon_Path(1,:))>=t_model.G.Lx/2-thck-dl)>0||sum(abs(Photon_Path(2,:))>=t_model.G.Ly/2-thck-dl)>0
                    plot3(Photon_Path(1,:),Photon_Path(2,:),-Photon_Path(3,:),'b'), hold on
                else
...
d_threshold = max(model.G.Lx/2-thck,model.G.Ly/2-thck);
a(d>d_threshold) = [];
w(d>d_threshold) = [];
s(d>d_threshold) = [];
d(d>d_threshold) = [];
...
M(Z>=zSurface&(abs(X)<=max(abs(X(:)))-double(parameters{10}))&(abs(Y)<=max(abs(Y(:)))-double(parameters{10}))) = double(parameters{2}); % standard (breast) tissue
M(sqrt( ...
