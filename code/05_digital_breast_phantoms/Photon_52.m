...
% Similar to Photon_45, but with corrected assignments for adipose (fat) and fibroglandular tissues.
        ...
        for i_HCT_Cn = 1:length(set_of_HCT_Cn)
            for i_beam_X = 1:length(set_of_beam_X)
                for i_Tumr_Z = 1:length(set_of_Tumr_Z)
                    ...
                    data(0.0<data&data<0.9) = 0.2; % Fibroglandular Tissue
                    data(0.9<=data        ) = 0.1; % Fat (Adipose Tissue)
                    data = round(data*10);
