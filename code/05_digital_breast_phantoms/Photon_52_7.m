...
% Similar to Photon_45_6, but brief
% analyzing simulated data: Testing an idea

clc
close all
load('DB/Photon_52_4_151.mat');
set(0,'DefaultFigureWindowStyle','docked')
set_of_lim = nan(length(set_of_percnt),length(set_of_versns),length(set_of_Tumr_Z),length(set_of_HCT_Cn));

code_num = 151;
p_threshold = 0.05;
wvlnt = 800; % sample wavelength (nm)
% Optical properties of breast and tumor at 800 nm [1-4]
...
plot(set_of_HCT_Cn,ones(size(set_of_mua)).*lit_data(1,1),'DisplayName','data #1','LineStyle','none','Marker','square','MarkerFaceColor',get_color(5),'MarkerSize',12), hold on
plot(set_of_HCT_Cn,ones(size(set_of_mua)).*lit_data(2,1),'DisplayName','data #2','LineStyle','none','Marker','square','MarkerFaceColor',get_color(6),'MarkerSize',12), hold on
plot(set_of_HCT_Cn,ones(size(set_of_mua)).*lit_data(3,1),'DisplayName','data #3','LineStyle','none','Marker','square','MarkerFaceColor',get_color(7),'MarkerSize',12), hold on
...
clearvars p_finish p_start Y_lim
...
% OD vs. mu_a, averaged across variants, for various source-detector distances
idx_d = 1:1:39; Y_Limit = [2 17]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_d = 1:length(idx_d)% OD/mu/d vs mua (several d)
        ...
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+1), subplot(3,3,i_Tumr_Z)
            ...
            plot(set_of_mua(idx_HCT_Cn),squeeze(mean(mean(y_refc,1),2)),'Color',TheColor,'HandleVisibility','off','LineWidth',0.5,'LineStyle','--'), hold on
            plot(set_of_mua(idx_HCT_Cn),squeeze(mean(mean(y_temp,1),2)),'Color',TheColor,'DisplayName',['d = ',num2str(d_centers(idx_d(i_d)))],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
...
clearvars i_Pd i_fig idx_d Y_Limit i_beam_X idx_HCT_Cn i_d i_Tumr_Z lit_data

% OD vs. mu_a, averaged across variants/percentages, for various source-detector distances
i_fig = 5; idx_d = 1:1:39; Y_Limit = [2 17]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_d = 1:length(idx_d)
    TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% OD as a function of source-detector distance, averaged over tissue types, for different mu_a values
idx_d = 1:1:39; Y_Limit = [2 17]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_HCT = 1:length(idx_HCT_Cn)
        TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+5), subplot(3,3,i_Tumr_Z)
            ...
            plot(d_centers(idx_d),squeeze(mean(mean(y_refc,1),2)),'Color',TheColor,'HandleVisibility','off','LineWidth',0.5,'LineStyle','--'), hold on
            plot(d_centers(idx_d),squeeze(mean(mean(y_temp,1),2)),'Color',TheColor,'DisplayName',['\mu_a = ',num2str(set_of_mua(idx_HCT_Cn(i_HCT))),''],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
...
% OD as a function of source-detector distance, averaged over tissue types and fat percentages, for different mu_a values
i_fig = 9; idx_d = 1:1:39; Y_Limit = [2 17]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_HCT = 1:length(idx_HCT_Cn)
    TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% 3-D plot of OD vs. mu_a & source-detector distance, averaged across variants
Z_Limit = [2 17]; i_beam_X = 1;
for i_Pd = 1:3
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_Pd+9), subplot(3,3,i_Tumr_Z)
        ...
        mesh(set_of_mua,d_centers,squeeze(mean(z_refc,2)).','EdgeColor','k','DisplayName','No Tumor','LineStyle','-'), hold on
        mesh(set_of_mua,d_centers,squeeze(mean(z_temp,2)).','EdgeColor','r','DisplayName','with Tumor','LineStyle','-'), hold on
        xlabel('\mu_a (cm^{-1})'), xlim([min(set_of_mua) max(set_of_mua)])
...
% 3-D plot of OD vs. mu_a & source-detector distance, averaged across variants/percentages
Z_Limit = [2 17]; i_beam_X = 1;
for i_Tumr_Z = 1:length(set_of_Tumr_Z)
    figure(4+9), subplot(3,3,i_Tumr_Z)
    ...
    mesh(set_of_mua,d_centers,squeeze(mean(mean(z_refc,1),2)).','EdgeColor','k','DisplayName','No Tumor','LineStyle','-'), hold on
    mesh(set_of_mua,d_centers,squeeze(mean(mean(z_temp,1),2)).','EdgeColor','r','DisplayName','with Tumor','LineStyle','-'), hold on
        xlabel('\mu_a (cm^{-1})'), xlim([min(set_of_mua) max(set_of_mua)])
    ylabel('d (cm)'), ylim([min(d_centers) 3.5])
    zlabel('OD (a.u.)'), zlim([min(Z_Limit) max(Z_Limit)])
    set(gca,'fontsize',12), legend('show','numColumns',1), view([-300 35])
...
% relative OD vs. mu_a, averaged across variants, for various source-detector distances
idx_d = 1:1:39; Y_Limit = [-1 +2.5]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_d = 1:length(idx_d)
        TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+13), subplot(3,3,i_Tumr_Z)
            p_comp =  set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn,5,idx_d(i_d));
            ...
            plot(set_of_mua(idx_HCT_Cn),squeeze(mean(mean(y_temp,1),2)),'Color',TheColor,'DisplayName',['d = ',num2str(d_centers(idx_d(i_d)))],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            xlabel('\mu_a (cm^{-1})'), xlim([min(set_of_mua) max(set_of_mua)])
            ylabel('\DeltaOD (a.u.)'), ylim([min(Y_Limit) max(Y_Limit)])
            set(gca,'fontsize',12), % legend('show','numColumns',1,'Orientation','horizontal'),
            title(['beamX = ',num2str(set_of_beam_X(i_beam_X)),' cm, Tumor depth = ',num2str(set_of_Tumr_Z(i_Tumr_Z)),' cm, Pd = ',num2str(set_of_percnt(i_Pd)),'%'])
            clearvars p_comp y_refc y_temp
        end
...
% relative OD vs. mu_a, averaged across variants/percentages, for various source-detector distances
i_fig = 17; idx_d = 1:1:39; Y_Limit = [-1 +2.5]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_d = 1:length(idx_d)
    TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% relative OD vs. source-detector distance, averaged across variants, for various mu_a
idx_d = 1:1:39; Y_Limit = [-1 +2.5]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_HCT = 1:length(idx_HCT_Cn)
        TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+17), subplot(3,3,i_Tumr_Z)
            p_comp = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),5,idx_d));
...
% relative OD vs. source-detector distance, averaged across variants/percentages, for various mu_a
i_fig = 21; idx_d = 1:1:39; Y_Limit = [-1 +2.5]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_HCT = 1:length(idx_HCT_Cn)
    TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% OD/mu_a/d vs. mu_a, averaged across variants, for various source-detector distances
idx_d = 1:1:39; Y_Limit = [0 50]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_d = 1:length(idx_d)
        TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+21), subplot(3,3,i_Tumr_Z)
            p_comp =      set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn,5,idx_d(i_d));
            y_refc = -log(set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn,4,idx_d(i_d)));
            y_temp = -log(set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn,2,idx_d(i_d)));
            y_temp(p_comp>p_threshold) = nan;

            plot(set_of_mua(idx_HCT_Cn),squeeze(mean(mean(y_refc,1),2))./set_of_mua(idx_HCT_Cn).'./d_centers(idx_d(i_d)),'Color',TheColor,'HandleVisibility','off','LineWidth',0.5,'LineStyle','--'), hold on
            plot(set_of_mua(idx_HCT_Cn),squeeze(mean(mean(y_temp,1),2))./set_of_mua(idx_HCT_Cn).'./d_centers(idx_d(i_d)),'Color',TheColor,'DisplayName',['d = ',num2str(d_centers(idx_d(i_d)))],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            xlabel('\mu_a (cm^{-1})'), xlim([min(set_of_mua) max(set_of_mua)])
...
% OD/mu_a/d vs. mu_a, averaged across variants/percentages, for various source-detector distances
i_fig = 25; idx_d = 1:1:39; Y_Limit = [0 50]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_d = 1:length(idx_d)
    TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% OD/mu_a/d vs. source-detector distance, averaged across variants, for various mu_a
idx_d = 1:1:39; Y_Limit = [0 50]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_HCT = 1:length(idx_HCT_Cn)
        TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+25), subplot(3,3,i_Tumr_Z)
            p_comp =     (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),5,idx_d));
            y_refc = -log(set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),4,idx_d));
            y_temp = -log(set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),2,idx_d));
            y_temp(p_comp>p_threshold) = nan;

            plot(d_centers(idx_d),squeeze(mean(mean(y_refc,1),2))./set_of_mua(idx_HCT_Cn(i_HCT))./d_centers(idx_d).','Color',TheColor,'HandleVisibility','off','LineWidth',0.5,'LineStyle','--'), hold on
            plot(d_centers(idx_d),squeeze(mean(mean(y_temp,1),2))./set_of_mua(idx_HCT_Cn(i_HCT))./d_centers(idx_d).','Color',TheColor,'DisplayName',['\mu_a = ',num2str(set_of_mua(idx_HCT_Cn(i_HCT))),''],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            xlabel('d (cm)'), xlim([min(d_centers) 3])
...
% OD/mu_a/d vs. source-detector distance, averaged across variants/percentages, for various mu_a
i_fig = 29; idx_d = 1:1:39; Y_Limit = [0 50]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_HCT = 1:length(idx_HCT_Cn)
    TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% relative OD/mu_a/d vs. mu_a, averaged across variants, for various source-detector distances
idx_d = 1:1:39; Y_Limit = [-5 +5]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_d = 1:length(idx_d)
        TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+29), subplot(3,3,i_Tumr_Z)
            p_comp =  set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn,5,idx_d(i_d));
            y_refc = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn,4,idx_d(i_d)));
            y_temp = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn,2,idx_d(i_d)));
            y_temp = -log(y_temp./y_refc);
            ...
            plot(set_of_mua(idx_HCT_Cn),zeros(size(set_of_mua(idx_HCT_Cn))),  'Color','k','HandleVisibility','off','LineWidth',0.5,'LineStyle','--'), hold on
            plot(set_of_mua(idx_HCT_Cn),squeeze(mean(mean(y_temp,1),2))./d_centers(idx_d(i_d))./set_of_mua(idx_HCT_Cn).','Color',TheColor,'DisplayName',['d = ',num2str(d_centers(idx_d(i_d)))],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            xlabel('\mu_a (cm^{-1})'), xlim([min(set_of_mua) max(set_of_mua)])
            ylabel('\DeltaOD/\mu_a/d (cm)'), ylim([min(Y_Limit) max(Y_Limit)])
            set(gca,'fontsize',12), % legend('show','numColumns',1,'Orientation','horizontal'),
            title(['beamX = ',num2str(set_of_beam_X(i_beam_X)),' cm, Tumor depth = ',num2str(set_of_Tumr_Z(i_Tumr_Z)),' cm, Pd = ',num2str(set_of_percnt(i_Pd)),'%'])
            clearvars p_comp y_refc y_temp
        end
...
% relative OD/mu_a/d vs. mu_a, averaged across variants/percentages, for various source-detector distances
i_fig = 33; idx_d = 1:1:39; Y_Limit = [-5 +5]; i_beam_X = 1; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_d = 1:length(idx_d)
    TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% relative OD/mu_a/d vs. source-detector distance, averaged across variants, for various mu_a
idx_d = 1:1:39; Y_Limit = [-5 +5]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_HCT = 1:length(idx_HCT_Cn)
        TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            figure(i_Pd+33), subplot(3,3,i_Tumr_Z)
            p_comp = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),5,idx_d));
            y_refc = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),4,idx_d));
            y_temp = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),2,idx_d));
            y_temp = -log(y_temp./y_refc);
...
% relative OD/mu_a/d vs. source-detector distance, averaged across variants/percentages, for various mu_a
i_fig = 37; idx_d = 1:1:39; Y_Limit = [-5 +5]; i_beam_X = 1; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_HCT = 1:length(idx_HCT_Cn)
    TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(3,3,i_Tumr_Z)
...
% to I_0
%                       percent      X     version      X     Beam_X       X     Tumor Z      X     HCT          X     N_bins
...
% Plot DPF for each case
figure(101)
for i_percent = 1:length(set_of_percnt)
    for i_versns = 1:4:length(set_of_versns)
        for i_beam_X = 1:length(set_of_beam_X)
            for i_Tumr_Z = 1:4:length(set_of_Tumr_Z)
                for i_HCT_Cn = 1:8:length(set_of_HCT_Cn)
                    p_comp = squeeze(set_of_I_s(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,5,:));
                    y_comp = squeeze(set_of_I_s(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,2,:));
                    y_comp(p_comp>p_threshold) = nan;
                    set_of_DPF_s(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,:) = -log(y_comp)./d_centers.'./set_of_mua(i_HCT_Cn);
                    plot(d_centers,y_comp,'LineWidth',2,...
                        'Color',[(i_percent-1)./(length(set_of_percnt)-1) (i_Tumr_Z-1)./(length(set_of_Tumr_Z)-1) (i_HCT_Cn-1)./(length(set_of_HCT_Cn)-1)], ...
                        'DisplayName',['%=',num2str(set_of_percnt(i_percent)),', Z=',num2str(set_of_Tumr_Z(i_Tumr_Z)),', HCT=',num2str(set_of_HCT_Cn(i_HCT_Cn))]), hold on
                    set(gca,'fontsize',24), legend('show','NumColumns',3)
                    clearvars p_comp y_comp
                end
            end
        end
    end
...
% Compute estimation error using shared DPF (per fat percentage) for tumor concentration
set_of_errrs = nan(size(set_of_I_s,1),size(set_of_I_s,2),size(set_of_I_s,3),size(set_of_I_s,4),size(set_of_I_s,5),size(set_of_I_s,7));
for i_percent = 1:length(set_of_percnt)
    for i_versns = 1:length(set_of_versns)
        for i_beam_X = 1:length(set_of_beam_X)
            for i_Tumr_Z = 1:length(set_of_Tumr_Z)
                for i_HCT_Cn = 1:length(set_of_HCT_Cn)
                    TheDPF = squeeze(nanmean(nanmean(set_of_DPF_s(i_percent,:,i_beam_X,i_Tumr_Z,i_HCT_Cn,:),2),1));
                    mua = -log(squeeze(set_of_I_s(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,2,:)))./d_centers.'./TheDPF;
                    set_of_errrs(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,:) = (mua-set_of_mua(i_HCT_Cn))./set_of_mua(i_HCT_Cn)*100;
                    clearvars TheColor TheDPF mua idx_of_d
                end
            end
        end
    end
end
clearvars i_percent i_versns i_beam_X i_Tumr_Z i_HCT_Cn idx_of_d
figure(102)
Y_Limit = [0 5]; X_limit = 3.5;
for i_beam_X = 1
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        idx_of_d = (0<=d_centers & d_centers<=X_limit);
        TheColor = 'b';
        subplot(3,3,i_Tumr_Z)
        the_sig = abs(squeeze(set_of_errrs(:,:,i_beam_X,i_Tumr_Z,:,idx_of_d))); the_sig = reshape(the_sig,length(set_of_percnt)*length(set_of_versns)*length(set_of_HCT_Cn),length(d_centers(idx_of_d))); hold on;
        do_shade(d_centers(idx_of_d),squeeze(nanmean(the_sig,1)),squeeze(nanstd(the_sig,[],1)),1,TheColor,['separate'])
        xlabel('d (cm)'), xlim([0 X_limit])
        ylabel('\Deltaerror (%)'), ylim([min(Y_Limit) max(Y_Limit)])
        set(gca,'fontsize',12), % legend('show','numColumns',1,'Orientation','horizontal'),
        title(['beamX = ',num2str(set_of_beam_X(i_beam_X)),' cm, Tumor depth = ',num2str(set_of_Tumr_Z(i_Tumr_Z)),' cm'])
        clearvars TheColor TheDPF mua idx_of_d the_sig
    end
end
clearvars i_percent i_versns i_beam_X i_Tumr_Z i_HCT_Cn

% Compute estimation error using shared DPF for tumor concentration
set_of_errrs = nan(size(set_of_I_s,1),size(set_of_I_s,2),size(set_of_I_s,3),size(set_of_I_s,4),size(set_of_I_s,5),size(set_of_I_s,7));
for i_percent = 1:length(set_of_percnt)
    for i_versns = 1:length(set_of_versns)
        for i_beam_X = 1:length(set_of_beam_X)
            for i_Tumr_Z = 1:length(set_of_Tumr_Z)
                for i_HCT_Cn = 1:length(set_of_HCT_Cn)
                    TheDPF = squeeze(nanmean(nanmean(set_of_DPF_s(:,:,i_beam_X,i_Tumr_Z,i_HCT_Cn,:),2),1));
                    mua = -log(squeeze(set_of_I_s(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,2,:)))./d_centers.'./TheDPF;
                    set_of_errrs(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,:) = (mua-set_of_mua(i_HCT_Cn))./set_of_mua(i_HCT_Cn)*100;
                    clearvars TheColor TheDPF mua idx_of_d
                end
...
for i_beam_X = 1
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        ...
        subplot(3,3,i_Tumr_Z)
        the_sig = abs(squeeze(set_of_errrs(:,:,i_beam_X,i_Tumr_Z,:,idx_of_d))); the_sig = reshape(the_sig,length(set_of_percnt)*length(set_of_versns)*length(set_of_HCT_Cn),length(d_centers(idx_of_d))); hold on;
        do_shade(d_centers(idx_of_d),squeeze(nanmean(the_sig,1)),squeeze(nanstd(the_sig,[],1)),1,TheColor,['shared'])
        xlabel('d (cm)'), xlim([0 X_limit])
        ylabel('\Deltaerror (%)'), ylim([min(Y_Limit) max(Y_Limit)])
        set(gca,'fontsize',12), legend('show','numColumns',1,'Orientation','horizontal','location','northwest'),
        title(['beamX = ',num2str(set_of_beam_X(i_beam_X)),' cm, Tumor depth = ',num2str(set_of_Tumr_Z(i_Tumr_Z)),' cm'])
        clearvars TheColor TheDPF mua idx_of_d the_sig
    end
end
clearvars i_percent i_versns i_beam_X i_Tumr_Z i_HCT_Cn set_of_errrs set_of_DPF_s X_limit Y_Limit
    ...
    for i_versns = 1:length(set_of_versns)
        for i_beam_X = 1:length(set_of_beam_X)
            for i_Tumr_Z = 1:length(set_of_Tumr_Z)
                for i_HCT_Cn = 1:length(set_of_HCT_Cn)
                    TheDPF = squeeze(nanmean(nanmean(set_of_DPF_s(i_percent,:,i_beam_X,i_Tumr_Z,i_HCT_Cn,:),2),1));
                    mua = -log(squeeze(set_of_I_s(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,2,:)./set_of_I_s(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,4,:)))./d_centers.'./TheDPF;
                    set_of_errrs(i_percent,i_versns,i_beam_X,i_Tumr_Z,i_HCT_Cn,:) = (mua-set_of_mua(i_HCT_Cn))./set_of_mua(i_HCT_Cn)*100;
                    clearvars TheColor TheDPF mua idx_of_d
                end
            end
...
for i_fig = 40:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_5_fig_',sprintf('%1.0f',i_fig),'.jpg' ],'jpeg'); end
end
...
function do_shade (x_avg,y_avg,y_std,the_line_width,the_color,the_legend)
x_avg = x_avg(:);
y_avg = y_avg(:);
y_std = y_std(:);
idx = isnan(y_avg) | isnan(y_std);
x_avg = x_avg(~idx);
y_avg = y_avg(~idx);
y_std = y_std(~idx);
...
min_y = y_avg - y_std;
max_y = y_avg + y_std;
fill([x_avg ; flip(x_avg)],[min_y ; flip(max_y)],the_color,'FaceAlpha',0.25,'HandleVisibility','off','EdgeColor','none'), hold on
plot( x_avg,                       y_avg,'Color',the_color,'LineWidth',the_line_width,'DisplayName',the_legend,'LineStyle','-'), hold on
end
