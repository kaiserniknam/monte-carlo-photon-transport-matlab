...
% Analyze simulated data to examine how changes in concentration affect the optical fluence on the surface (per Dr. Das’s request).
% same as Photon_52_8 but calcuate error due to no-tumor DPF

clc
close all
load('DB/Photon_52_4_151.mat');
set(0,'DefaultFigureWindowStyle','docked')
set_of__PQ = nan(length(set_of_percnt),length(set_of_versns),length(set_of_Tumr_Z),length(set_of_HCT_Cn),2);
set_of_DPF = nan(length(set_of_percnt),length(set_of_versns),length(set_of_Tumr_Z),length(set_of_HCT_Cn),N_bins);
...
i_beam_X = 1; Y_Limit = [0 40];
for i_Pd = 1:length(set_of_percnt)
    for i_HCT = 1:length(set_of_HCT_Cn)
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            % figure(i_Pd), subplot(3,3,i_Tumr_Z)
            for i_ver = 1:length(set_of_versns)
                y_temp = -log(set_of_I_s(i_Pd,i_ver,i_beam_X,i_Tumr_Z,i_HCT,2,:));
                X = d_centers.'; Y = squeeze(y_temp)./set_of_mua(i_HCT);
                X = X(~isnan(Y)&~isinf(Y));
                Y = Y(~isnan(Y)&~isinf(Y));
                coeffs = polyfit(X, Y, 1);
                set_of__PQ(i_Pd,i_ver,i_Tumr_Z,i_HCT,1) = coeffs(1);
                set_of__PQ(i_Pd,i_ver,i_Tumr_Z,i_HCT,2) = coeffs(2);
                set_of_DPF(i_Pd,i_ver,i_Tumr_Z,i_HCT,1:length(Y)) = Y./X;
                clearvars X Y coeffs
            end
            clearvars i_ver
            % if i_HCT==1
            %     TheColor = [0 7/8 0];
            %     plot(d_centers,squeeze(mean(set_of_DPF(i_Pd,:,i_Tumr_Z,i_HCT,:),2)),'Color',TheColor,'DisplayName',['no tumor'],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            % else
            %     TheColor = [(i_HCT-1)/(length(set_of_HCT_Cn)-1),0,1-(i_HCT-1)/(length(set_of_HCT_Cn)-1)];
            %     plot(d_centers,squeeze(mean(set_of_DPF(i_Pd,:,i_Tumr_Z,i_HCT,:),2)),'Color',TheColor,'DisplayName',['no tumor'],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            % end
            ...
            % title(['Tumor depth = ',num2str(set_of_Tumr_Z(i_Tumr_Z)),' cm, Pd = ',num2str(set_of_percnt(i_Pd)),'%'])

            clearvars p_comp y_refc y_temp X Y p q coeffs
        end
        clearvars TheColor
    end
end
clearvars i_Pd i_fig idx_d Y_Limit i_beam_X idx_HCT_Cn i_d i_Tumr_Z i_HCT


i_beam_X = 1; i_Tumr_Z = 1; Y_Limit = [-20 +20];
set_of_pqmuas = [];
set_of_error = nan(length(set_of_percnt),length(set_of_versns),length(set_of_HCT_Cn),N_bins);
for i_Pd = 1:length(set_of_percnt)
    for i_ver = 1:length(set_of_versns)
        for i_HCT = 2:length(set_of_HCT_Cn)
            p      = squeeze(mean(mean(mean(mean(set_of__PQ(:,:,i_Tumr_Z,i_HCT,1),1),2),3),4));
            q      = squeeze(mean(mean(mean(mean(set_of__PQ(:,:,i_Tumr_Z,i_HCT,2),1),2),3),4));
            % thespf = squeeze(mean(mean(mean(mean(set_of_DPF(:,:,i_Tumr_Z,i_HCT,:),1),2),3),4));
            set_of_pqmuas = [set_of_pqmuas ; [set_of_mua(i_HCT) set_of_mus(i_HCT) set_of__PQ(i_Pd,i_ver,i_Tumr_Z,i_HCT,1) set_of__PQ(i_Pd,i_ver,i_Tumr_Z,i_HCT,2)]];

            figure(4), subplot(3,3,(i_Pd-1)*3+i_ver)
            y_temp = -log(set_of_I_s(i_Pd,i_ver,i_beam_X,i_Tumr_Z,i_HCT,2,:));
            X = d_centers.'; Y = squeeze(y_temp)./X./(p+q./X);
            set_of_error(i_Pd,i_ver,i_HCT,:) = (Y-set_of_mua(i_HCT))./set_of_mua(i_HCT)*100;
            if i_HCT~=1
                TheColor = [(i_HCT-1)/(length(set_of_HCT_Cn)-1),0,1-(i_HCT-1)/(length(set_of_HCT_Cn)-1)];
                plot(X,squeeze(set_of_error(i_Pd,i_ver,i_HCT,:)),'Color',TheColor,'DisplayName',['C = ',num2str(set_of_HCT_Cn(i_HCT)),'%'],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            end
            xlabel('d (cm)'), xlim([min(d_centers) 3])
            ylabel('estimation error (%)'), ylim([min(Y_Limit) max(Y_Limit)])
            set(gca,'fontsize',12), % legend('show','numColumns',1,'Orientation','horizontal'),
            title(['Pd = ',num2str(set_of_percnt(i_Pd)),' %, ver = ',num2str(set_of_versns(i_ver)),'%'])
            grid on
        end
        clearvars p_comp y_refc y_temp X Y p q coeffs
        clearvars TheColor
...
% calc p & q
[p_coeff] = fitPlaneModel(log(set_of_pqmuas(:,1)), log(set_of_pqmuas(:,2)), log(set_of_pqmuas(:,3)));
disp(num2str(['C = ',num2str(exp(p_coeff(1))),', a = ',num2str(p_coeff(2)),', b = ',num2str(p_coeff(3))]))
[q_coeff] = fitPlaneModel(log(set_of_pqmuas(:,1)), log(set_of_pqmuas(:,2)), log(set_of_pqmuas(:,4)));
disp(num2str(['C = ',num2str(exp(q_coeff(1))),', a = ',num2str(q_coeff(2)),', b = ',num2str(q_coeff(3))]))
...
for i_Pd = 1:length(set_of_percnt)
    for i_ver = 1:length(set_of_versns)
        for i_HCT = 2:length(set_of_HCT_Cn)
            ...
            p = exp(p_coeff(1)).*((set_of_mua(i_HCT)).^(p_coeff(2))).*((set_of_mus(i_HCT)).^(p_coeff(3)));
            q = exp(q_coeff(1)).*((set_of_mua(i_HCT)).^(q_coeff(2))).*((set_of_mus(i_HCT)).^(q_coeff(3)));


            figure(5), subplot(3,3,(i_Pd-1)*3+i_ver)
            y_temp = -log(set_of_I_s(i_Pd,i_ver,i_beam_X,i_Tumr_Z,i_HCT,2,:));
            X = d_centers.'; Y = squeeze(y_temp)./X./(p+q./X);
            set_of_error(i_Pd,i_ver,i_HCT,:) = (Y-set_of_mua(i_HCT))./set_of_mua(i_HCT)*100;
            if i_HCT~=1
            ...
            xlabel('d (cm)'), xlim([min(d_centers) 2.2])
            ylabel('estimation error (%)'), ylim([min(Y_Limit) max(Y_Limit)])
            set(gca,'fontsize',12), % legend('show','numColumns',1,'Orientation','horizontal'),
            title(['Pd = ',num2str(set_of_percnt(i_Pd)),' %, ver = ',num2str(set_of_versns(i_ver)),'%'])
            grid on
...
A = [ones(numel(X), 1), X(:), Y(:)];  % Design matrix: [1 X Y]
b = Z(:);                                % Observed values
