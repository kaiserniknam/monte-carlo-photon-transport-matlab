...
for i_Pd = 1:3
    for i_HCT = length(idx_HCT_Cn):-1:1
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            ...
            if i_HCT==1
                TheColor = [0 7/8 0];
                plot(d_centers(idx_d),squeeze(mean(mean(y_temp,1),2)),'Color',TheColor,'DisplayName',['no tumor'],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            else
                TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
                plot(d_centers(idx_d),squeeze(mean(mean(y_temp,1),2)),'Color',TheColor,'DisplayName',['C = ',num2str(set_of_HCT_Cn(i_HCT)),'%'],'LineWidth',2,'Marker','none','MarkerFaceColor',TheColor,'MarkerEdgeColor','none','LineStyle','-','MarkerSize',6), hold on
            end
