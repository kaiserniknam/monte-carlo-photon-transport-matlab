...
% Similar to Photon_45_6: but for C - C_{min}
...
idx_d = 1:1:39; Y_Limit = [-2 +7]; i_beam_X = 2; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_d = 1:length(idx_d)
        ...
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            ...
            y_refc = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,2,         2,idx_d(i_d)));
...
i_fig = 5; idx_d = 1:1:39; Y_Limit = [-2 +7]; i_beam_X = 2; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_d = 1:length(idx_d)
    TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(2,2,i_Tumr_Z)
        p_comp =  set_of_I_s(:,:,i_beam_X,i_Tumr_Z,idx_HCT_Cn,5,idx_d(i_d));
        y_refc = (set_of_I_s(:,:,i_beam_X,i_Tumr_Z,2         ,2,idx_d(i_d)));
...
idx_d = 1:1:39; Y_Limit = [-2 +7]; i_beam_X = 2; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_HCT = 1:length(idx_HCT_Cn)
        ...
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
            ...
            y_refc = (set_of_I_s(i_Pd,1:3,i_beam_X,i_Tumr_Z,2,                2,idx_d));
...
i_fig = 9; idx_d = 1:1:39; Y_Limit = [-2 +7]; i_beam_X = 2; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_HCT = 1:length(idx_HCT_Cn)
    TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(2,2,i_Tumr_Z)
        p_comp = (set_of_I_s(:,:,i_beam_X,i_Tumr_Z,idx_HCT_Cn(i_HCT),5,idx_d));
        y_refc = (set_of_I_s(:,:,i_beam_X,i_Tumr_Z,2,                2,idx_d));
...
% OD_relative vs mua (several d)
idx_d = 1:1:39; Y_Limit = [-0.5 +1.5]; i_beam_X = 2; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_d = 1:length(idx_d)
        TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
...
i_fig = 13; idx_d = 1:1:39; Y_Limit = [-0.5 +1.5]; i_beam_X = 2; idx_HCT_Cn = 2:length(set_of_HCT_Cn);
for i_d = 1:length(idx_d)
    TheColor = [(i_d-1)/(length(idx_d)-1),0,1-(i_d-1)/(length(idx_d)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(2,2,i_Tumr_Z)
...
% OD_relative vs d (several mua)
idx_d = 1:1:39; Y_Limit = [-0.5 +1.5]; i_beam_X = 2; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_Pd = 1:3
    for i_HCT = 1:length(idx_HCT_Cn)
        TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
        for i_Tumr_Z = 1:length(set_of_Tumr_Z)
...
i_fig = 16; idx_d = 1:1:39; Y_Limit = [-0.5 +1.5]; i_beam_X = 2; idx_HCT_Cn = 2:1:length(set_of_HCT_Cn);
for i_HCT = 1:length(idx_HCT_Cn)
    TheColor = [(i_HCT-1)/(length(idx_HCT_Cn)-1),0,1-(i_HCT-1)/(length(idx_HCT_Cn)-1)];
    for i_Tumr_Z = 1:length(set_of_Tumr_Z)
        figure(i_fig), subplot(2,2,i_Tumr_Z)
...
for i_fig = 16:-1:1, figure(i_fig), saveas(gcf,['Photon_',num2str(code_num),'_5_fig_',sprintf('%1.0f',i_fig),'.jpg' ],'jpeg'); end
end
