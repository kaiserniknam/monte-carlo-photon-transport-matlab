...
for i_g = 1:length(set_of_g)
    ...
    % I vs. d
    i_fig = 2; fun_x = @mean; fun_y = @sum; TheOutFun = @(x)(-log(x)); TheLineStyle = '-'; TheMarker = 'none'; x_label = 'd (cm)'; y_label = 'OD (a.u.)'; Therloess = 0.0; blnFit = false;
    TheLegend = ['TiO_2 = ',num2str(set_of_g(i_g)),' g'];
    % (diffuse)
    ...
    [~,~,~] = make_a_subplot(i_fig,i_subplot,index_in,x_temp,y_temp,fun_x,fun_y,mua,mus,TheColor,TheOutFun,TheLineStyle,TheMarker,Therloess,blnFit,TheLegend);
    if get_plot(mua,mus)
