...
% this version: Reading Arnab data - April 18, 2025, test reading
...
TheTest = nan(3,32);
for i_TiO2 = 1:3
    TheTest(i_TiO2,01) = set_of_TiO2(i_TiO2);
    TheTest(i_TiO2,02) = TheBase(i_TiO2);

    i_Trzn = 1; i_India = 1; x_temp = squeeze(TheData(i_Trzn,i_India,i_TiO2,:)).';
    TheTest(i_TiO2,03:07) = [x_temp,std(x_temp),mean(x_temp)];
    i_Trzn = 1; i_India = 2; x_temp = squeeze(TheData(i_Trzn,i_India,i_TiO2,:)).';
    TheTest(i_TiO2,08:12) = [x_temp,std(x_temp),mean(x_temp)];
    i_Trzn = 1; i_India = 3; x_temp = squeeze(TheData(i_Trzn,i_India,i_TiO2,:)).';
    TheTest(i_TiO2,13:17) = [x_temp,std(x_temp),mean(x_temp)];

    i_Trzn = i_Trzn_selected; i_India = 1; x_temp = squeeze(TheData(i_Trzn,i_India,i_TiO2,:)).';
    TheTest(i_TiO2,18:22) = [x_temp,std(x_temp),mean(x_temp)];
    i_Trzn = i_Trzn_selected; i_India = 2; x_temp = squeeze(TheData(i_Trzn,i_India,i_TiO2,:)).';
    TheTest(i_TiO2,23:27) = [x_temp,std(x_temp),mean(x_temp)];
    i_Trzn = i_Trzn_selected; i_India = 3; x_temp = squeeze(TheData(i_Trzn,i_India,i_TiO2,:)).';
    TheTest(i_TiO2,28:32) = [x_temp,std(x_temp),mean(x_temp)];

end
disp(num2str(TheTest,'%2.8f  '))
