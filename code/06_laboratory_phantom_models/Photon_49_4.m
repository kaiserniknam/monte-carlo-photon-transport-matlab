...
code_num = 39; N = 1000; L = 6; % piecewise path length
...
for i_X = 1:length(set_of_beam_X)
    for i_Y = 1:length(set_of_beam_Y)
        if (i_X~=1||i_Y~=1)
            ...
            set_of_pq__sets = nan(length(set_of_N_points)+1,N,2+1);
            ...
            for i_P = 1:length(set_of_N_points)
                N_p = set_of_N_points(i_P);
                for i_rnd = 1:N
                    idx = randperm(length(X),N_p);
                    ...
                    ft_og = p - q./X;
                    r_squared = calc_rsquared(-Y./X,ft_og);
                    set_of_pq__sets(i_P,i_rnd,:) = [p q r_squared];
                    clearvars Xt Yt coeffs p q r_squared idx
                end
                clearvars N_p
            end
            clearvars i_rnd i_P
            ...
            A = squeeze(set_of_pq__sets(:,:,1));
            edges = linspace(0,5,nbins+1);
            counts = zeros(nbins,size(A,1));
            for j = 1:size(A,1)
                counts(:,j) = histcounts(A(:,j),edges,"Normalization","probability");
            end
            subplot(2,2,1), bar3(counts), shading interp
            xlabel('# of samples'); ylabel('P bins'); zlabel('Frequency');
            title('3-D Histogram of p vs. # of samples');
            set(gca,'fontsize',12), colormap jet
            clearvars A edges counts j

            figure(2),
            nbins = 51; % number of bins
            A = squeeze(set_of_pq__sets(:,:,2));
            edges = linspace(0,10,nbins+1);
            counts = zeros(nbins,size(A,1));
            for j = 1:size(A,1)
                counts(:,j) = histcounts(A(:,j),edges,"Normalization","probability");
            end
            subplot(2,2,2), bar3(counts), shading interp
            xlabel('# of samples'); ylabel('q bins'); zlabel('Frequency');
            title('3-D Histogram of q vs. # of samples');
            set(gca,'fontsize',12), colormap jet
            clearvars A edges counts j

            figure(2),
            nbins = 51; % number of bins
            A = squeeze(set_of_pq__sets(:,:,3));
            edges = linspace(0,1,nbins+1);
            counts = zeros(nbins,size(A,1));
            for j = 1:size(A,1)
                counts(:,j) = histcounts(A(:,j),edges,"Normalization","probability");
            end
            subplot(2,1,2), bar3(counts), shading interp
            xlabel('# of samples'); ylabel('r^2 bins'); zlabel('Frequency');
            title('3-D Histogram of r^2 vs. 3 of samples');
            set(gca,'fontsize',12)
            clearvars A edges counts j
            ...
            figure(3),
            X  = set_of_N_points;
            Y  = squeeze(mean(squeeze(set_of_pq__sets(:,:,1)),2)).';
            sd = squeeze( std(squeeze(set_of_pq__sets(:,:,1)),[],2)).';
            subplot(2,2,1), plot(X,Y(end).*ones(size(X)),'LineWidth',2,'Color','b','DisplayName','true value'); hold on
            subplot(2,2,1), plot(X,Y(1:end-1),'LineWidth',2,'Color','r','DisplayName','p'); hold on
            subplot(2,2,1), fill([X fliplr(X)],[Y(1:end-1)+sd(1:end-1) fliplr(Y(1:end-1)-sd(1:end-1))],'b','FaceAlpha',0.2,'EdgeColor','none','HandleVisibility','off'); hold on
            xlabel('# of samples'); ylabel('p'); title('the evolution of p');
            set(gca,'fontsize',12), hold off
            clearvars X Y sd

            figure(3),
            X  = set_of_N_points;
            Y  = squeeze(mean(squeeze(set_of_pq__sets(:,:,2)),2)).';
            sd = squeeze( std(squeeze(set_of_pq__sets(:,:,2)),[],2)).';
            subplot(2,2,2), plot(X,Y(end).*ones(size(X)),'LineWidth',2,'Color','b','DisplayName','true value'); hold on
            subplot(2,2,2), plot(X,Y(1:end-1),'LineWidth',2,'Color','r','DisplayName','p'); hold on
            subplot(2,2,2), fill([X fliplr(X)],[Y(1:end-1)+sd(1:end-1) fliplr(Y(1:end-1)-sd(1:end-1))],'b','FaceAlpha',0.2,'EdgeColor','none','HandleVisibility','off'); hold on
            xlabel('# of samples'); ylabel('q'); title('the evolution of q');
            set(gca,'fontsize',12), hold off
            clearvars X Y sd

            figure(3),
            X  = set_of_N_points;
            Y  = squeeze(mean(squeeze(set_of_pq__sets(:,:,3)),2)).';
            sd = squeeze( std(squeeze(set_of_pq__sets(:,:,3)),[],2)).';
            subplot(2,1,2), plot(X,Y(end).*ones(size(X)),'LineWidth',2,'Color','b','DisplayName','true value'); hold on
            subplot(2,1,2), plot(X,Y(1:end-1),'LineWidth',2,'Color','r','DisplayName','p'); hold on
            subplot(2,1,2), fill([X fliplr(X)],[Y(1:end-1)+sd(1:end-1) fliplr(Y(1:end-1)-sd(1:end-1))],'b','FaceAlpha',0.2,'EdgeColor','none','HandleVisibility','off'); hold on
            xlabel('# of samples'); ylabel('r^2'); title('the evolution of r^2'); ylim([0 1])
            set(gca,'fontsize',12), hold off
            clearvars X Y sd
