...
for i_g = 1:length(set_of_g)
    n_g = set_of_g(i_g);
    n = 1.333.*sqrt((1 + 0.0005293.*n_g)./(1 - 0.0002647.*n_g));
    g = 0.9;
    mua = 0.01;
    mus = 39.835.*n_g;
    the_filename = ['Photon_40_g_',sprintf('%.0f',set_of_g(i_g)),'.mat' ];
    if ~exist(the_filename,'file')
        ...
        save  (the_filename, ...
            'x', 'y', 'z', 'd', 's', 'w', 'c', 'a', 'no_of_photons', 'M_raw','n_g','n','g','mua','mus','beam_X','beam_Y','beam_phi','beam_tht')

        clearvars x y z d s w c a no_of_photons M_raw
        clearvars mua mus n g n_g
    end
