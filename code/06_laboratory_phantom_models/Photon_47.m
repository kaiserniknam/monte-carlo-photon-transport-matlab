...
%
% Phantom Dimensions: 10 cm (L) × 10 cm (W) × 5 cm (H)
% Embedded Tube:
%  Diameter: 3 mm
%  Depth from top surface: 1 cm
%  Length of signal inside: ~4 cm (centered laterally in the phantom)
%  Volume filled: ~2–4 mL (?)
%
% Background Medium: Agar gel mixed with TiO₂ at concentrations of:
%  0.5, 2.5, 5.0, and 7.5 g/L
%
% Signal Medium (inside the tube): India ink solutions at:
% 0.05%, 0.25%, 1%, 5%, 10%, 20%
% Note: 1% = 1 mL ink per 1 L of water
%
% Optical Setup:
% Wavelength: 760 nm
% Source-detector distance: 6 cm (both placed on the top surface)
% Signal region located at the center beneath source-detector path
...
nExamplePaths = 500; % number of example photon paths
% Optical properties of TiO2 in agar
mua_TiO2 = .003; mus_TiO2 = 16; g_TiO2 = 0.880; n_TiO2 = 1.36;
% Optical properties of India Ink
mua_indk = 12.0; mus_indk = 3.; g_indk = 0.700; n_indk = 1.34;
...
for i_TiO2 = 1:length(set_of_TiO2)
    for i_indk = 1:length(set_of_indk)
        ...
        save  (['Photon_47_TiO2_',num2str(the_TiO2),'_Indik_',num2str(the_indk),'.mat' ], ...
            'p_in','w_in', ...
            'p_ot','w_ot', ...
            'T_mua_TiO2', 'T_mus_TiO2', 'T_g_TiO2', 'T_n_TiO2', ...
            'T_mua_indk', 'T_mus_indk', 'T_g_indk', 'T_n_indk', ...
